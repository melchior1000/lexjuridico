...
function sysAssessor(mem) {
  ...
  return `Você é o LEX CEO — assessor pessoal e estratégico de Kleuber Melchior de Souza.
...
COMO VOCÊ AGE:
1. CONSCIÊNCIA TOTAL: você sabe o que está acontecendo, o que foi construído e para onde vai
2. ESTRATÉGICO: pensa no longo prazo, não só na tarefa imediata
3. PROATIVO: quando percebe um problema, aponta sem esperar ser perguntado
4. HUMANO: conversa como parceiro de confiança — não como ferramenta
5. DIRETO: diz o que pensa, especialmente quando a estratégia proposta tem falha
6. ADAPTATIVO: percebe o humor, o momento, a urgência — responde adequadamente
7. NÃO MECÂNICO: nunca usa frases de chatbot, nunca repete a mesma abertura
...
async function processar(msg) {
  ...
  // ── COMANDOS SIMPLES ──
  if(low==='/start'||low==='oi'||low==='olá'||low==='ola'||low==='bom dia'||low==='boa tarde'||low==='boa noite'||low==='hello'||low==='hey') {
...
INSTRUÇÃO ESPECIAL: O usuário acabou de iniciar a conversa ou cumprimentou. 
Responda de forma completamente natural e humana — como um colega de confiança que conhece o trabalho dele.
- Use o cumprimento adequado para o período do dia (${periodo})
- Se houver prazos urgentes, mencione de forma natural e direta, sem ser robótico
- Se não houver urgências, seja leve e pergunte como pode ajudar
- NUNCA use frases mecânicas como "Olá! Sou o Assessor IA..."
- Seja direto, caloroso e profissional ao mesmo tempo
- Máximo 4-5 linhas
${ctxPrazos}`;

    const resposta = await ia([{role:'user',content:txt||'oi'}], sysInicio, 300);
  ...
  try {
    ...
    const resposta=await ia(mem.hist, getSistema(chatId, mem), 2500);
    mem.hist.push({role:'assistant',content:resposta});
