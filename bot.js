...
const AUTORIZADOS = { '696337324': { nome:'Admin', perfil:'admin', ok:true } };
...
function sysAssessor(mem) {
  ...
  return `Você é o Assessor IA do ${ESCRITORIO.nome}.
${ESCRITORIO.responsavel?'Responsável: '+ESCRITORIO.responsavel+(ESCRITORIO.registro?' — '+ESCRITORIO.registro:''):''}
Peças assinadas pelo responsável informado (ou profissional indicado no momento).
Data: ${hoje}
${contexto}
...
async function processar(msg) {
  ...
  console.log('MSG ['+chatId+'] tipo:'+msg.chat.type+':', txt||'[arquivo]');
  // Log de atividade
  logAtividade('juridico', chatId, txt?'mensagem':'arquivo', txt.substring(0,100));
  
  // Em grupos/canais — só responde se for mencionado ou for comando
  ...
  const isAutorizado = AUTORIZADOS[chatId]?.ok || chatId === CHAT_ID;
  if(isGrupo && !isMencao) return; // Em grupos só responde quando mencionado
  if(!isAutorizado && !isGrupo) return; // Em privado só autorizados
  ...
  if(msg.document) {
    ...
    if(!isOk){await env('Formato não suportado. Envie PDF, DOCX, TXT ou imagem (JPG, PNG).',tId,chatId);return;}
  ...
  if(low==='/start'||low==='oi'||low==='olá'||low==='ola') {
    ...
    let m='Lex Assessor IA — '+ESCRITORIO.nome+'\n\n';
    if(urg.length) {
      m+='⚠️ PRAZOS URGENTES:\n';
  ...
  const palavrasCumprimento = ['cumpri','cumprido','cumpriu','protocolei','protocolo feito','peticionei','petição feita','recurso protocolado','juntei','juntada feita','já fiz','foi feito','foi protocolado','foi enviado','enviamos'];
  const temCumprimento = palavrasCumprimento.some(p=>low.includes(p));
  
  if(temCumprimento && processos.length > 0) {
    // Tenta identificar o processo mencionado
    ...
    if(procMenc) {
      ...
      // Atualiza processo — limpa prazo e registra andamento
      const idx = processos.findIndex(x=>x.id===procMenc.id);
      if(idx>=0) {
        const prazoAnterior = processos[idx].prazo || '';
        processos[idx].prazo = ''; // limpa prazo cumprido
        processos[idx].prazoReal = '';
        if(!processos[idx].andamentos) processos[idx].andamentos = [];
        processos[idx].andamentos.unshift({
          data: hoje,
          txt: 'Prazo cumprido — '+(prazoAnterior?'era '+prazoAnterior+' — ':'')+txt.substring(0,200)
        });
        // Salva no Supabase
        try {
          await sbUpsert('processos_sync', {
            processo_id: String(procMenc.id),
            nome: procMenc.nome,
            prazo: '',
            ultimo_andamento: 'Prazo cumprido em '+hoje+': '+txt.substring(0,150),
            atualizado_em: new Date().toISOString()
          }, 'processo_id');
        } catch(e) { /* fallback silencioso */ }
        logAtividade('juridico', chatId, 'prazo_cumprido', procMenc.nome+' — '+hoje);
        await env('✅ Registrado!\n\nProcesso: '+procMenc.nome+'\nData: '+hoje+'\n\n'+txt.substring(0,200)+'\n\nAndamento adicionado ao processo. Prazo limpo.', tId, chatId);
      }
    } else {
      // Não identificou o processo — pede para informar
      await env('Ótimo! Prazo cumprido.\n\nPara registrar no sistema, informe o nome do processo:\n\nEx: "Cumpri o prazo do Varejão"\n\nOu acesse o Lex e adicione o andamento manualmente.', tId, chatId);
    }
    return;
  }
...
http.createServer(async(req,res)=>{
  ...
  // POST /api/analisar — analisa PDF/imagem base64
...
env('Lex Assessor IA — '+ESCRITORIO.nome+'\n\nMemória ativa — lembro de toda a conversa.\n\n/novo — atender novo cliente\nEnvie PDF, DOCX ou foto para análise\nDescreva qualquer caso ou problema\n/prazos /processos /prep /ajuda').then(()=>poll());
