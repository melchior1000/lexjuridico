...
async function today(){
 ...
 for(const p of ps){const d=days(p);if(d<=0&&!p.deadline_truth)items.push({rank:0,tone:'critical',who:p.nome||p.partes||p.numero||'Processo',meta:p.numero||'Processo sem número',title:d<0?'Prazo cadastrado vencido':'Prazo cadastrado para hoje',detail:'Prazo legado ainda sem verdade jurídica assinada. Confirme a situação antes de agir.',label:'Ver processo',action:openProcess(String(p.id))})}
