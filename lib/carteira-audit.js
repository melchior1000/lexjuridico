...
async function applyCnjCorrection({processStore,processId,numero,now=new Date(),actor='advogado'}){
  ...
  const saved=await processStore.mutate(ps=>{
    ...
    andamentos.unshift({data:hojeBrasil(now),txt:'[LEX] Número corrigido'+(anterior?' de "'+anterior+'"':'')+' para '+formatCnj(cnj)+' (confirmado por '+actor+')',origem:'lex'});
...
// Corrige e já executa o resto: publicações órfãs do Diário entram no processo
// e, se o tribunal estiver ligado, o LEX consulta andamentos e partes.
async function correctAndRefresh({processStore,processId,numero,now=new Date(),actor='advogado',dbReq=null,pje=null}){
  const out=await applyCnjCorrection({processStore,processId,numero,now,actor});
  const cnj=String(out.processo.numero||'').replace(/\D/g,'').slice(0,20);
  const extra={publicacoes:0,tribunal:'nao_conectado',diario_ok:null,diario_erro:null};
  try{
    const djen=await require('./djen-monitor').attachOrphans({processStore,sbReq:dbReq,cnj});
    extra.publicacoes=djen.casadas;
    extra.diario_ok=djen.falhas.length===0;
    if(djen.falhas.length)extra.diario_erro=djen.falhas.length+' publicação(ões) não puderam ser vinculadas agora';
  }catch(error){
    extra.diario_ok=false;
    extra.diario_erro=error.message||'falha ao consultar o Diário';
  }
  const sigla=tribunalFromCnj(cnj);
  if(pje?.config?.configurado&&pje.client&&sigla&&pje.client.tribunais().includes(sigla)){
    try{const r=await require('./pje-process-sync').syncProcessesFromPje({client:pje.client,processStore,processId:out.processo.id,now});extra.tribunal=r.atualizados.length?'atualizado':'falhou'}
    catch{extra.tribunal='falhou'}
  }
  const atual=((await processStore.read()).processes||[]).find(x=>String(x.id)===String(out.processo.id))||out.processo;
  return{processo:atual,anterior:out.anterior,sigla,...extra};
}
function correctionMessage(r){
  const nome=r.processo.nome||r.processo.id;
  const lines=['✅ Número de "'+nome+'" gravado: '+r.processo.numero+(r.anterior?' (antes: '+r.anterior+')':'')+'.'];
  if(r.publicacoes)lines.push('Trouxe '+r.publicacoes+' publicação(ões) do Diário para o processo.');
  if(r.diario_erro)lines.push('⚠️ Diário: '+r.diario_erro+'. O número foi corrigido, mas não considero as publicações sincronizadas.');
  lines.push({atualizado:'Tribunal ('+r.sigla+'): andamentos e partes atualizados.',falhou:'Tribunal ('+r.sigla+') não respondeu agora; tento de novo na próxima atualização.',nao_conectado:(r.sigla||'O tribunal')+' ainda não está ligado ao LEX: o acompanhamento segue pelo Diário.'}[r.tribunal]||'');
  const ult=(r.processo.andamentos||[])[0];
  if(ult&&!/^\[LEX\] Número corrigido/.test(String(ult.txt||'')))lines.push('Último andamento: '+String(ult.data||'').split('-').reverse().join('/')+' — '+String(ult.txt||'').replace(/^\[\w+\]\s*/,'').slice(0,160));
...
module.exports={cnjCheckDigits,cnjValid,formatCnj,isAdministrative,digitDistance,auditCarteira,auditMessage,applyCnjCorrection,correctAndRefresh,correctionMessage,readOrphans,suggestionsFor};
