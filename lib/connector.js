...
async function captureMovement(store,body) {
  ...
  return store.mutate(ps=>{
    ...
    // Registra apenas quando um movimento específico foi capturado de forma assistida.
    // Este timestamp NÃO prova leitura integral do processo nem ausência de novidades.
    p.ultimo_movimento_capturado_em=new Date().toISOString();
