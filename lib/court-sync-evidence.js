...
function mintCourtSyncEvidence({readingId}={},deps={}){
  ...
  if(!verifyReadingLogEntry(reading,{integrityKey:deps.integrityKey}))throw new CourtSyncEvidenceError('reading_provenance_invalid');
  if(reading.process_id==null||String(reading.process_id).trim()==='')throw new CourtSyncEvidenceError('reading_process_id_missing');
  ...
  const evidence=Object.freeze({
    ...
    ok:true,
    explicit_no_change:reading.explicit_no_change===true,
    movement_received:reading.movement_received===true,
    response_hash:reading.integridade.response_hash,
    binding_hmac:reading.integridade.binding_hmac
  });
  MINTED.add(evidence);
  return evidence;
}
function isCourtSyncEvidence(obj){return typeof obj==='object'&&obj!==null&&MINTED.has(obj)}
module.exports={mintCourtSyncEvidence,isCourtSyncEvidence,CourtSyncEvidenceError,OFFICIAL_SOURCES};
