...
async function buildDeadlineSuggestion({communication,aiAnalyze,feriados=[],calendarioVerificado=false}={}){
  ...
  if(!text)return{status:'sem_texto',legal_truth:false,source_hash};
  ...
  if(!chosen.candidate)return{...chosen,legal_truth:false,source_hash,candidates};
...
module.exports={cleanText,sourceHash,explicitDeadlineCandidates,defaultRegime,parseAiJson,chooseCandidateWithAi,buildDeadlineSuggestion};
