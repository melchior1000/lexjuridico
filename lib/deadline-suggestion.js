...
function sourceHash(communication={}){
  return crypto.createHash('sha256').update([
    cleanText(communication.texto),
    String(communication.data_disponibilizacao||''),
    String(communication.tribunal||''),
    String(communication.tipo||'')
...
function explicitDeadlineCandidates(text){
  const raw=String(text||''),out=[],seen=new Set(),ranges=[];
  const patterns=[
    ...
    /\b(?:contestação|contestacao|impugnação|impugnacao|réplica|replica|recurso|embargos?|manifestação|manifestacao)\b[\s\S]{0,40}?\b(?:em|no\s+prazo\s+de)\s+(\d{1,3})\s+dias?\s*(uteis|úteis|corridos|consecutivos)?\b/giu
  ];
  for(const re of patterns){
    ...
    while((m=re.exec(raw))){
      ...
      const key=dias+'|'+modo+'|'+cleanText(m[0]).toLowerCase();
      if(seen.has(key))continue;seen.add(key);ranges.push({start,end});
      out.push({index:out.length,dias,modo,trecho:excerpt(raw,start,end)});
    }
  }
...
function defaultRegime(communication={}){
  ...
  if(/^TRT|TRABALH/.test(tribunal))return'clt';
  return'unknown';
}
...
async function chooseCandidateWithAi({text,candidates,communication,aiAnalyze}={}){
  ...
  const rawIndex=parsed?.candidate_index;
  if(rawIndex===null||rawIndex===undefined||rawIndex==='')return{status:'ambigua',candidate:null,regime:'unknown',confidence:Number(parsed?.confidence)||0};
...
async function buildDeadlineSuggestion({communication,aiAnalyze,feriados=[],calendarioVerificado=false}={}){
  const text=cleanText(communication?.texto),source_hash=sourceHash(communication);
  if(!text)return{suggestion_version:SUGGESTION_VERSION,status:'sem_texto',legal_truth:false,source_hash};
  const candidates=explicitDeadlineCandidates(text);
  let chosen,ai_error=null;
  try{
    chosen=await chooseCandidateWithAi({text,candidates,communication,aiAnalyze});
  }catch(error){
    ai_error=cleanText(error?.message||'Falha no provedor de IA').slice(0,500);
    chosen=await chooseCandidateWithAi({text,candidates,communication,aiAnalyze:null});
  }
  if(!chosen.candidate)return{suggestion_version:SUGGESTION_VERSION,...chosen,legal_truth:false,source_hash,ai_error,candidates};
  ...
  return{
    suggestion_version:SUGGESTION_VERSION,status:proposta?'proposta_calculada':chosen.status,
    legal_truth:false,source_hash,dias:c.dias,modo:c.modo,regime:chosen.regime,confidence:chosen.confidence,
    trecho:c.trecho,justificativa:chosen.justificativa||null,ai_error,
...
module.exports={SUGGESTION_VERSION,cleanText,sourceHash,explicitDeadlineCandidates,defaultRegime,parseAiJson,chooseCandidateWithAi,buildDeadlineSuggestion};
