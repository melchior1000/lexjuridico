...
function mintDeadlineTruth(refs={},deps={}){
  ...
  if(auth.process_id&&reading.process_id&&String(auth.process_id)!==String(reading.process_id))throw new DeadlineTruthError('authorization_process_mismatch');
  const truth=Object.freeze({legal_truth:true,process_id:reading.process_id,due_at:reading.due_at,source,observed_at:reading.observed_at,reading_id:readingId,authorization_id:authorizationId,authorized_by:auth.human_id,authorized_at:auth.authorized_at,minted_at:Number.isFinite(deps.now)?deps.now:Date.now()});
  MINTED.add(truth);
  return truth;
}
