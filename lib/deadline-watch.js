...
function civilDate(value){
  ...
  const s=String(value||'');const m=s.match(/^(\d{4}-\d{2}-\d{2})/);return m&&flow.date(m[1])?m[1]:null;
}
...
function truthForProcess(truths,p){return(Array.isArray(truths)?truths:[]).find(t=>isLegalTruth(t)&&String(t.process_id)===String(p.id))||null}
function freshnessOf(p,now=new Date(),syncEvidence=null,truth=null){
  const evidence=isCourtSyncEvidence(syncEvidence)&&String(syncEvidence.process_id)===String(p.id)?syncEvidence:null;
  const legal=isLegalTruth(truth)&&String(truth.process_id)===String(p.id)?truth:null;
...
function watchlist(processes,now=new Date(),syncEvidences=[],truths=[]){
  return(Array.isArray(processes)?processes:[]).filter(p=>!flow.CLOSED.has(String(p.status||'').toUpperCase())).map(p=>{
    const f=freshnessOf(p,now,evidenceForProcess(syncEvidences,p),truthForProcess(truths,p)),last=Array.isArray(p.andamentos)&&p.andamentos[0]?p.andamentos[0]:null;
...
async function openOfficeWatch({processes,session,intent_id,actor_id='LEX',syncFn,now=new Date()}){
  ...
  if(block||typeof syncFn!=='function'||!canReachCourt(session)){const warning=block||'LEX não atualizou prazo: sem acesso ao TJ.';return Object.freeze({ok:false,intent_id:intent,synced:false,warning,items:watchlist(processes,now).map(i=>({...i,warning})),ledger_event:{...base,result:'blocked',payload:{...base.payload,warning}}})}
  ...
  const truths=(Array.isArray(result?.deadline_truths)?result.deadline_truths:[]).filter(isLegalTruth);
  ...
  return Object.freeze({ok:explicit,intent_id:intent,synced:explicit,warning,items:watchlist(after,now,evidences,truths),ledger_event:{...base,result:explicit?'synced':'failed',causation_id:result?.causation_id||null,payload:{...base.payload,source,observed_at:observedAt,official_sync:explicit}}});
}
