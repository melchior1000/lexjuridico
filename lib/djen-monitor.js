...
async function suggestPendingDeadlines(sbReq,rows=[],options={}){
  ...
  for(const row of Array.isArray(rows)?rows:[]){
    ...
    if(row?.prazo_sugestao&&row.prazo_sugestao.status&&row.prazo_sugestao.source_hash===currentHash){out.push(row);continue}
    try{
      ...
      out.push(persisted||{...row,prazo_sugestao:suggestion});
    }catch(error){
...
module.exports={parseOabs,communicationRow,upsertCommunication,markCommunication,listPendingMint,persistDeadlineSuggestion,suggestPendingDeadlines,readSyncState,saveSyncState,syncWindows,syncDjen};
