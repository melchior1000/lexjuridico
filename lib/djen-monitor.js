...
function communicationRow(item,oab,uf,audit={}){
  ...
  return{
    ...
    status:cancel?'cancelada':'nova',observado_em:audit.responded_at||new Date().toISOString(),endpoint:audit.endpoint||null,request_id:audit.request_id||null
  };
}
...
async function syncDjen({processStore,sbReq,oabs=parseOabs(),now=new Date(),clientOptions={}}={}){
  ...
  for(const cfg of oabs){
    ...
    for(const item of result.items){
      const audit=result.audits[result.audits.length-1]||{},row=communicationRow(item,cfg.oab,cfg.uf,audit);
