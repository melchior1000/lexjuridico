...
async function attachOrphans({processStore,sbReq,cnj}){
  ...
  const rows=rowsFromResult(await sbReq('GET','djen_comunicacoes',null,{cnj:'eq.'+digits,status:'eq.orfa',order:'data_disponibilizacao.asc',limit:'50'}),'Ler publicações órfãs do processo');
  const out={casadas:0,falhas:[]};
  for(const row of rows){
    try{
      const applied=await applyPjeMovement({processStore,origem:'djen'},{cnj:digits,data:row.data_disponibilizacao||new Date().toISOString().slice(0,10),andamento_texto:[row.tipo||'Comunicação DJEN',row.tribunal||'',row.texto||''].filter(Boolean).join(' — '),observed_at:row.observado_em,djen_id:row.djen_id});
...
module.exports={attachOrphans,parseOabs,parseOabInput,setOabsProvider,communicationRow,upsertCommunication,markCommunication,listPendingMint,persistDeadlineSuggestion,suggestPendingDeadlines,readSyncState,saveSyncState,syncWindows,syncDjen};
