...
async function porOab(numeroOab,ufOab,janela=ontemHoje(),deps={}){
  const vistos=new Set(),items=[],audits=[];let pages=0;
  for(const variant of oabVariants(numeroOab)){
    for(let page=1;page<=Number(deps.maxPages||100)&&items.length<Number(deps.maxItems||5000);page++){
      ...
      pages++;audits.push({...r.audit,numero_oab:variant,uf_oab:String(ufOab).toUpperCase()});
      if(!r.items.length)break;
      for(const item of r.items){const key=itemKey(item);if(vistos.has(key))continue;vistos.add(key);items.push({...item,textoSeguro:sanitizeText(item.texto)})}
      if(r.items.length<50||items.length>=r.count)break;
    }
  }
  return{items,pagesConsulted:pages,audits,janela};
}
