...
async function readWithVision(buffer,{mimeType='image/jpeg',filename='documento',apiKey=process.env.OPENAI_API_KEY,model=process.env.OPENAI_VISION_MODEL||'gpt-5',request=requestOpenAI}={}){
  ...
  const instruction=`Leia o documento brasileiro com rigor pericial. Se qualquer dado estiver cortado, escuro, borrado ou ambíguo, NÃO complete. Responda somente JSON válido com: {"tipo_documento":"extrato_bancario|holerite|cnis|contrato|rg|cpf|cnh|rg_cpf_unificado|fgts|trct|ctps|outro","confianca":"alta|media|baixa","texto_legivel":"transcrição fiel do que é possível ler","evidencias":[{"pagina":null,"data":"","linha":"","campo":"","valor":""}],"observacoes":""}. Para valores monetários, só inclua valor quando houver evidência visual clara e origem identificável. Se não conseguir ler com segurança, use confianca baixa e deixe campos vazios.`;
  ...
  const critical=CRITICAL_TYPES.has(String(obj?.tipo_documento||'').toLowerCase());
  const monetaryEvidence=evidences.filter(e=>String(e?.valor||'').match(/\d/)).every(e=>e?.pagina!=null||String(e?.data||'').trim()||String(e?.linha||'').trim());
  const ok=confidence==='alta'&&text.length>=20&&(!critical||monetaryEvidence);
  return {ok,confidence,reason:ok?'visao_confirmada':confidence!=='alta'?'confianca_insuficiente':!text?'texto_vazio':'evidencia_insuficiente',source:'openai_vision',type:String(obj?.tipo_documento||'outro'),text,evidences,observations:String(obj?.observacoes||'').slice(0,1000),raw:obj};
}
...
module.exports={CRITICAL_TYPES,normalizeConfidence,assessTextQuality,extractPdfText,readWithVision,readDocument,mustBlockReading,unreadMessage,extractResponseText};
