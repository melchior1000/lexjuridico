...
// Porta independente de frescor para leituras judiciais.
// ok:true e necessario, mas nunca suficiente.
...
function checkFreshness(reading, opts = {}) {
  ...
  if (reading.observed_at == null) return { fresh: false, reason: 'observed_at_missing' };
  const observedMs = toEpochMs(reading.observed_at);
  if (observedMs === null) return { fresh: false, reason: 'observed_at_invalid' };
