...
const INTRO = 'Olá! Meu nome é LEX, sou o assistente virtual do escritório LEX Jurídico e trabalho sob orientação do Dr. Kleuber. Como posso ajudar?';
...
function intakeDecision(text, data = {}, history = []) {
  ...
  const replies = {
    ...
    existing_case: prefix + 'por segurança, informações do caso dependem da autorização do Dr. Kleuber.' + (name ? ' Se tiver o número do processo, pode informar; se não tiver, tudo bem.' : ' Me diga seu nome e, se tiver, o número do processo.'),
    ...
    identify: 'Prazer, ' + name + '. Como posso ajudar no seu atendimento?',
  ...
  if (reply === context.lastReply && !['greeting','identity','ack'].includes(kind)) {
    reply = prefix + 'entendi sua nova mensagem. Vou juntá-la ao pedido para o Dr. Kleuber; a resposta sobre o assunto depende dele.';
  }
