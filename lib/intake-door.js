...
// ou posição do escritório sem autorização expressa do advogado titular.
// Nomes do escritório e do titular vêm de lib/office-identity (white-label).
const {getIdentity, titularKeywords} = require('./office-identity');
function intro(id = getIdentity()) {
  return 'Olá, meu nome é ' + id.assistente + '. Sou o assistente virtual do ' + id.escritorioFrase + ' e trabalho sob orientação ' + id.doTitular + '. Em que posso te ajudar?';
}
...
function classify(text, data = {}) {
  ...
  const nomes = ['advogado', 'advogada', 'doutor', 'doutora', 'responsavel', ...titularKeywords()].join('|');
  if (new RegExp('(?:falar|conversar|contato|chamar|chame|chama|falo|fale).*\\b(' + nomes + ')\\b|\\b(' + nomes + ') (?:esta|ta|pode|disponivel)|me passa o advogad').test(n)) return 'lawyer';
...
function intakeDecision(text, data = {}, history = []) {
  ...
  const T = id.titularTratado, AO = id.aoTitular;
  ...
  const replies = {
    ...
    identity: firstContact ? INTRO : (name ? 'Sou o ' + id.assistente + ', ' + name + '. Trabalho no ' + id.escritorioFrase + ' e estou acompanhando seu atendimento. Em que posso te ajudar?' : 'Sou o ' + id.assistente + '. Trabalho no ' + id.escritorioFrase + ' e estou acompanhando seu atendimento. Em que posso te ajudar?'),
    lawyer: prefix + 'entendi. Vou levar seu pedido ' + AO + ' e aguardar a orientação antes de te dar uma posição.' + identify,
    sensitive: prefix + 'entendi o ponto. Como assessor, posso organizar as informações, mas não decido nem assumo posição pelo escritório. Vou submeter isso ' + AO + ' antes de responder.' + identify,
    urgent: prefix + 'entendi a urgência. Vou encaminhar seu relato ' + AO + ' com prioridade e aguardar a orientação antes de te passar qualquer posição.' + identify,
    existing_case: name ? name + ', por segurança, os dados, o andamento, os prazos e qualquer posição sobre o processo dependem da autorização ' + id.doTitular + '. Posso organizar as informações: me informe o número do processo ou, se não tiver, me diga do que se trata.' : 'Por segurança, os dados, o andamento, os prazos e qualquer posição sobre o processo dependem da autorização ' + id.doTitular + '. Posso organizar as informações: me diga seu nome e, se tiver, o número do processo.',
    pericia: prefix + 'posso receber e organizar o material para o setor de perícia. Cálculo, conclusão ou valor só serão apresentados após revisão e autorização ' + id.doTitular + '.' + identify,
    ...
    administrative: prefix + 'recebi o contato administrativo. Vou organizar as informações e submeter ' + AO + ' antes de assumir qualquer compromisso pelo escritório.' + identify,
    new_case: prefix + 'posso organizar seu pedido de atendimento e preparar as informações para ' + (id.titular ? 'o ' + T : 'o advogado responsável') + '.' + (name ? ' Me conte o que aconteceu e o que você precisa.' : ' Me diga seu nome e, em poucas palavras, o que aconteceu.'),
    ...
    general: name ? name + ', pode me contar os fatos. Vou organizar seu relato e, antes de qualquer orientação ou posição do escritório, submeto ' + AO + '.' : 'Pode me contar o que aconteceu. Vou organizar seu relato para ' + (id.titular ? 'o ' + T : 'o advogado responsável') + '; antes de qualquer orientação ou posição do escritório, preciso da autorização ' + id.doTitular + '.',
  ...
  if (reply === context.lastReply && !['ack'].includes(kind)) {
    reply = name ? name + ', entendi. Pode me passar o próximo fato ou documento; eu organizo e submeto ' + AO + ' antes de qualquer posição.' : 'Entendi. Pode me passar o próximo fato ou documento; eu organizo e submeto ' + AO + ' antes de qualquer posição.';
  }
  ...
  return {
    ...
    ownerRule:'LEX pode acolher, perguntar, organizar e preparar. Não pode orientar juridicamente, decidir, negociar, prometer, concluir perícia ou assumir posição do escritório sem autorização ' + id.doTitular + '.'
  };
}

module.exports = {intro, classify, extractName, intakeContext, intakeDecision, authorityFor};
// Compatibilidade: INTRO reflete a identidade configurada no momento da leitura.
Object.defineProperty(module.exports, 'INTRO', {enumerable:true, get:() => intro()});
