...
// Ele não substitui o responsável jurídico, não assume compromissos e não envia
// conteúdo jurídico ou posição do escritório sem autorização expressa.
const OFFICE_NAME = String(process.env.ESCRITORIO_NOME || 'LEX Jurídico').trim() || 'LEX Jurídico';
const RESPONSIBLE_NAME = String(process.env.ESCRITORIO_RESP || process.env.LEX_OPERATOR_NAME || 'responsável do escritório').trim() || 'responsável do escritório';
const INTRO = `Olá, meu nome é LEX. Sou o assistente virtual do escritório ${OFFICE_NAME} e trabalho sob orientação de ${RESPONSIBLE_NAME}. Em que posso te ajudar?`;
...
function responsibleLabel() { return RESPONSIBLE_NAME; }

function classify(text, data = {}) {
  const n = normalize(text);
  const m = data.message || {};
  const configured = normalize(RESPONSIBLE_NAME);
  const asksConfigured = configured && configured !== 'responsavel do escritorio' && n.includes(configured) && /\b(falar|conversar|contato|chamar|chame|chama|falo|fale|esta|ta|pode|disponivel)\b/.test(n);
  if (/\b(urgente|prisao|preso|mandado|liminar)\b|(?:audiencia|prazo|vence) (?:hoje|amanha)/.test(n)) return 'urgent';
  if (asksConfigured || /(?:falar|conversar|contato|chamar|chame|chama|falo|fale).*\b(kleuber|advogado|doutor|responsavel)\b|\b(kleuber|doutor|advogado) (?:esta|ta|pode|disponivel)|me passa o advogado/.test(n)) return 'lawyer';
...
function intakeDecision(text, data = {}, history = []) {
  ...
  const replies = {
    ...
    identity: firstContact ? INTRO : (name ? 'Sou o LEX, ' + name + '. Trabalho no escritório ' + OFFICE_NAME + ' e estou acompanhando seu atendimento. Em que posso te ajudar?' : 'Sou o LEX. Trabalho no escritório ' + OFFICE_NAME + ' e estou acompanhando seu atendimento. Em que posso te ajudar?'),
    lawyer: prefix + 'entendi. Vou levar seu pedido a ' + responsible + ' e aguardar a orientação antes de te dar uma posição.' + identify,
    sensitive: prefix + 'entendi o ponto. Como assessor, posso organizar as informações, mas não decido nem assumo posição pelo escritório. Vou submeter isso a ' + responsible + ' antes de responder.' + identify,
    urgent: prefix + 'entendi a urgência. Vou encaminhar seu relato a ' + responsible + ' com prioridade e aguardar a orientação antes de te passar qualquer posição.' + identify,
    existing_case: name ? name + ', por segurança, os dados, o andamento, os prazos e qualquer posição sobre o processo dependem da autorização de ' + responsible + '. Posso organizar as informações: me informe o número do processo ou, se não tiver, me diga do que se trata.' : 'Por segurança, os dados, o andamento, os prazos e qualquer posição sobre o processo dependem da autorização de ' + responsible + '. Posso organizar as informações: me diga seu nome e, se tiver, o número do processo.',
    pericia: prefix + 'posso receber e organizar o material para o setor de perícia. Cálculo, conclusão ou valor só serão apresentados após revisão e autorização de ' + responsible + '.' + identify,
    ...
    administrative: prefix + 'recebi o contato administrativo. Vou organizar as informações e submeter a ' + responsible + ' antes de assumir qualquer compromisso pelo escritório.' + identify,
    new_case: prefix + 'posso organizar seu pedido de atendimento e preparar as informações para ' + responsible + '.' + (name ? ' Me conte o que aconteceu e o que você precisa.' : ' Me diga seu nome e, em poucas palavras, o que aconteceu.'),
    ...
    general: name ? name + ', pode me contar os fatos. Vou organizar seu relato e, antes de qualquer orientação ou posição do escritório, submeto a ' + responsible + '.' : 'Pode me contar o que aconteceu. Vou organizar seu relato para ' + responsible + '; antes de qualquer orientação ou posição do escritório, preciso de autorização.',
  ...
  if (reply === context.lastReply && !['ack'].includes(kind)) {
    reply = name ? name + ', entendi. Pode me passar o próximo fato ou documento; eu organizo e submeto a ' + responsible + ' antes de qualquer posição.' : 'Entendi. Pode me passar o próximo fato ou documento; eu organizo e submeto a ' + responsible + ' antes de qualquer posição.';
  }
  ...
  return {
    ...
    ownerRule:'LEX pode acolher, perguntar, organizar e preparar. Não pode orientar juridicamente, decidir, negociar, prometer, concluir perícia ou assumir posição do escritório sem autorização de ' + responsible + '.'
  };
}

module.exports = {INTRO, OFFICE_NAME, RESPONSIBLE_NAME, classify, extractName, intakeContext, intakeDecision, authorityFor};
