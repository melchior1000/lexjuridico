...
const INTRO = 'Sou o LEX, assistente virtual do escritório LEX Jurídico e trabalho sob orientação do Dr. Kleuber. O que você precisa?';
...
function intakeDecision(text, data = {}, history = []) {
  ...
  const replies = {
    ...
    existing_case: name ? name + ', você tem o número do processo ou prefere me dizer do que se trata?' : 'Certo. Me diga seu nome e, se tiver, o número do processo. Se não tiver, pode me dizer do que se trata.',
  ...
  if (reply === context.lastReply && !['identity','ack'].includes(kind)) {
