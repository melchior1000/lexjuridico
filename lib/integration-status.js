...
function publicWhatsappDecision(text, data = {}) {
  ...
  if (/prisao|preso|mandado|liminar|audiencia (hoje|amanha)|prazo (hoje|amanha)|vence hoje|vence amanha|bloqueio urgente|urgente/.test(n)) return {kind:'urgent',escalate:true,archive:false,reply:'Entendi. Registrei como urgente e o responsável foi avisado. Se houver prazo, audiência ou documento, envie aqui para eu deixar tudo organizado.'};
  if (/quero falar com (o |a )?(advogado|doutor|responsavel)|falar com (o |a )?(advogado|doutor)|preciso do advogado|me passa o advogado/.test(n)) return {kind:'lawyer',escalate:true,archive:false,reply:'Certo. Registrei seu pedido para falar com o responsável. Informe seu nome completo e, em uma frase, o assunto para eu deixar o retorno organizado.'};
  if (/quanto (eu )?(ganho|recebo)|valor (do|de) (acordo|processo)|proposta de acordo|aceito acordo|estrategia|o que voce acha que eu ganho|honorario|honorarios/.test(n)) return {kind:'sensitive',escalate:true,archive:false,reply:'Esse ponto precisa de análise do responsável. Registrei seu pedido. Informe seu nome e, se houver, o número do caso para o retorno correto.'};
  if (/meu processo|meu caso|andamento|numero do processo|n[uú]mero do processo|sentenca|recurso|peticao|audiencia|prazo do processo/.test(n)) return {kind:'existing_case',escalate:true,archive:false,reply:'Neste canal eu não abro processo automaticamente por segurança. Me passe seu nome e o número do caso; o responsável retorna com a informação correta.'};
  if (/cobranca|vivo|claro|tim|operadora|fornecedor|financeiro|boleto|fatura|nota fiscal|nf-e|nfe/.test(n)) return {kind:'administrative',escalate:false,archive:true,reply:'Recebido. Registrei o contato administrativo. Se houver boleto, nota ou documento, pode enviar por aqui.'};
  if (hasDocument || hasAudio || hasImage) return {kind:'media',escalate:false,archive:false,reply:'Recebi o arquivo. Para eu organizar corretamente, informe seu nome e em uma frase a que assunto ele se refere.'};
  if (/trabalhist|demissao|demitid|rescisao|acidente de trabalho|horas extras|empresa nao pagou|quero processar|preciso entrar com acao|preciso de advogado|advogado trabalhista|advogado civil|advogado tributario/.test(n)) return {kind:'new_case',escalate:false,archive:false,reply:'Consigo iniciar o atendimento. Me informe seu nome completo, sua cidade e, em uma frase, o que aconteceu.'};
  if (/quem (e|eh)|de quem|esse numero|este numero/.test(n)) return {kind:'identity',escalate:false,archive:true,reply:'Olá! Este é o WhatsApp do Lex Jurídico. Se precisar falar com o escritório, diga seu nome ou empresa e o assunto.'};
  if (/^(oi|ola|bom dia|boa tarde|boa noite|opa|al[oô])\b/.test(n)) return {kind:'greeting',escalate:false,archive:false,reply:'Olá! Você falou com o Lex Jurídico. Diga seu nome ou empresa e, em poucas palavras, como posso ajudar.'};
...
async function _sendEvolutionText(number, text, instance, {url, key, request = requestJson} = {}) {
  ...
  try {
    const result=await request(evolutionEndpoint(url,'message/sendText/'+encodeURIComponent(instance)),{method:'POST',headers:{apikey:key},timeoutMs:15000,data:{number:String(number),text:String(text).substring(0,4000)}});
...
async function handleWhatsappOperatorCommand(body, instance, options = {}) {
  const data=body?.data||body||{}; const text=whatsappMessageText(data);
  ...
  if(/^\/ajuda\s*$/i.test(text)){await _sendEvolutionText(operator,'Comandos privados do LEX:\n/recepcao — contatos aguardando\n/historico NUMERO — últimas mensagens do contato\n/responder NUMERO mensagem — responder pelo WhatsApp do escritório\n/arquivar NUMERO — retirar da fila',instance,{url:cfg.url,key:cfg.key,request});return true;}
  if(/^\/recepcao\s*$/i.test(text)){const rows=await store.list({status:'aguardando_advogado',limit:10});await _sendEvolutionText(operator,_listReceptionText(rows),instance,{url:cfg.url,key:cfg.key,request});return true;}
  const historyMatch=text.match(/^\/historico\s+(\+?[\d\s().-]+)\s*$/i);
  if(historyMatch){let target;try{target=brazilMobile(historyMatch[1]);}catch{target=null;}if(!target){await _sendEvolutionText(operator,'Número inválido. Use: /historico 5561999999999',instance,{url:cfg.url,key:cfg.key,request});return true;}const rows=typeof store.history==='function'?await store.history(target,{limit:15}):[];await _sendEvolutionText(operator,_historyText(rows,target),instance,{url:cfg.url,key:cfg.key,request});return true;}
  const replyMatch=text.match(/^\/responder\s+(\+?[\d\s().-]+)\s+([\s\S]+)$/i);
  if(replyMatch){let target;try{target=brazilMobile(replyMatch[1]);}catch{target=null;}if(!target){await _sendEvolutionText(operator,'Número inválido. Use: /responder 5561999999999 sua mensagem',instance,{url:cfg.url,key:cfg.key,request});return true;}const replyText=replyMatch[2].trim();const ok=await _sendEvolutionText(target,replyText,instance,{url:cfg.url,key:cfg.key,request});if(ok&&typeof store.appendEvent==='function') await store.appendEvent({numero:target,nome:'Contato',direcao:'saida_operador',texto:replyText,classe:'geral',nivel:'atencao'});await _sendEvolutionText(operator,ok?`Resposta enviada para ${target}.`:`Não consegui confirmar o envio para ${target}.`,instance,{url:cfg.url,key:cfg.key,request});return true;}
  const archiveMatch=text.match(/^\/arquivar\s+(\+?[\d\s().-]+)\s*$/i);
  if(archiveMatch){let target;try{target=brazilMobile(archiveMatch[1]);}catch{target=null;}const archived=target?await store.archive(target):false;await _sendEvolutionText(operator,archived?`Contato ${target} arquivado.`:'Contato não encontrado na fila.',instance,{url:cfg.url,key:cfg.key,request});return true;}
  return false;
}
...
async function publicWhatsappReception(body, instance, options = {}) {
  ...
  if(replied){if(typeof store.appendEvent==='function') await store.appendEvent({numero:sender,nome:name,direcao:'saida_lex',texto:decision.reply,classe:item.classe||'geral',nivel:level});await _sendEvolutionText(operator,`[LEX] respondeu a ${name} (${sender}): ${decision.reply}`,instance,{url:cfg.url,key:cfg.key,request});}
  return replied;
}

const publicReceptionTurnBuffer=createReceptionTurnBuffer({dispatch:(body,instance)=>publicWhatsappReception(body,instance),delayMs:1200});
...
function incomingWhatsappMessage(body,instance){
  ...
  const valid=typeof data.key?.id==='string'&&!!data.key.id&&/^\d+(?::\d+)?@s\.whatsapp\.net$/.test(data.key?.remoteJid||'')&&!!data.message&&typeof data.message==='object';
  if(!valid) return false;
  const mode=whatsappAccessMode(data.key.remoteJid,process.env.LEX_OPERATOR_WHATSAPP||'');
  if(mode==='public'){queueMicrotask(()=>publicReceptionTurnBuffer.enqueue(body,instance).catch(()=>{}));return false;}
  if(mode==='operator'&&/^\/(recepcao|historico|responder|arquivar|ajuda)\b/i.test(whatsappMessageText(data))){queueMicrotask(()=>handleWhatsappOperatorCommand(body,instance).catch(()=>{}));return false;}
  return true;
}
...
module.exports={brazilMobile,sameBrazilWhatsappNumber,whatsappAccessMode,publicWhatsappReply,publicWhatsappDecision,publicWhatsappReception,handleWhatsappOperatorCommand,requestJson,evolutionEndpoint,whatsappStatus,telegramStatus,webhookAuthStatus,incomingWhatsappMessage};
