...
async function publicWhatsappReception(body, instance, options = {}) {
  ...
  if (typeof store.appendEvent==='function') await store.appendEvent({numero:sender,nome,direcao:'entrada',texto:original,classe:item.classe||'geral',nivel:level});
  ...
  if (replied) {
    if (typeof store.appendEvent==='function') await store.appendEvent({numero:sender,nome,direcao:'saida_lex',texto:decision.reply,classe:item.classe||'geral',nivel:level});
