...
async function publicWhatsappReception(body, instance, options = {}) {
  ...
  await _sendEvolutionText(operator, `[RECEPÇÃO] ${name} (${sender}): ${safeOriginal}`, instance, {url:cfg.url,key:cfg.key,request});
...
// Filtra antes do motor jurídico. Quando há operador configurado, apenas ele
// entra no pipeline privado; os demais números recebem recepção pública isolada.
...
async function whatsappStatus({url, key, instance, number, enabled = true}, getJson = requestJson) {
  ...
  try {
    ...
    // A Evolution 2.3.7 trata JIDs brasileiros com e sem o nono digito como equivalentes.
...
module.exports = {
  ...
  publicWhatsappReception, requestJson, evolutionEndpoint, whatsappStatus,
