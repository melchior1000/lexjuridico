...
async function publicWhatsappReception(body, instance, options = {}) {
  ...
  if (decision.escalate || item.urgente) {
    const marker = item.urgente || decision.kind==='urgent' ? '[URGENTE]' : '[ATENÇÃO]';
    const reason = decision.kind==='existing_case' ? 'possível processo existente' : decision.kind==='lawyer' ? 'pediu advogado' : decision.kind==='sensitive' ? 'tema sensível' : item.classe;
    await _sendEvolutionText(operator, `${marker} ${name} (${sender}) — ${reason}: ${safeOriginal}`, instance, {url:cfg.url,key:cfg.key,request});
  }
  return _sendEvolutionText(sender, decision.reply, instance, {url:cfg.url,key:cfg.key,request});
}
