...
async function handleWhatsappOperatorCommand(body, instance, options = {}) {
  ...
  if(namedReply){
    const rows=await store.list({status:'aguardando_advogado',limit:50});
    const norm=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
    const wanted=norm(namedReply.nome);
    const matches=rows.filter(row=>{const name=norm(row.nome);return name===wanted||name.startsWith(wanted+' ')||wanted.startsWith(name+' ');});
    if(matches.length!==1){
