...
async function _sendEvolutionText(number, text, instance, {url, key, request = requestJson} = {}) {
  ...
  try {
    ...
    console.warn(`[WhatsApp Evolution] sendText falhou status=${status} destino=${destino}`);
    return false;
  }
}
...
async function handleWhatsappOperatorCommand(body, instance, options = {}) {
  ...
  if(namedReply){
    let matches;
    try {
      matches=await store.findWaitingByName(namedReply.nome);
      if(!Array.isArray(matches)) throw new Error('Busca de destinatario invalida');
    } catch {
      await _sendEvolutionText(operatorTarget,'Não consegui confirmar todos os contatos com esse nome. Nada foi enviado. Tente novamente ou informe o número do destinatário.',instance,{url:cfg.url,key:cfg.key,request});
      return true;
    }
    if(matches.length!==1){
...
module.exports={brazilMobile,sameBrazilWhatsappNumber,whatsappAccessMode,whatsappIdentity,publicWhatsappReply,publicWhatsappDecision,publicWhatsappReception,handleWhatsappOperatorCommand,requestJson,evolutionEndpoint,whatsappStatus,telegramStatus,webhookAuthStatus,incomingWhatsappMessage};
