...
// Compositor de resposta da recepção (lib/reception-ai.js), registrado pelo servidor
// quando há IA configurada. Sem registro, a recepção usa as frases fixas.
let receptionComposer = null;
function setReceptionComposer(fn){receptionComposer=typeof fn==='function'?fn:null;}
...
async function publicWhatsappReception(body, instance, options = {}) {
  ...
  if(audioFailure){
    ...
    // A IA escreve a conversa dentro dos limites da decisão; a decisão (classe, setor,
    // escalonamento, aprovação) continua do código. Sem IA, vale a frase fixa.
    const composer=options.compose||receptionComposer;
    if(composer&&text){
      try{const composed=await composer({text,decision,history});if(composed?.reply)decision={...decision,reply:composed.reply,origemResposta:composed.origem};}
      catch(e){console.warn('[Recepcao IA] compositor falhou; resposta fixa: '+(e?.message||e));}
    }
  }
...
module.exports={setReceptionComposer,brazilMobile,sameBrazilWhatsappNumber,whatsappAccessMode,whatsappIdentity,publicWhatsappReply,publicWhatsappDecision,publicWhatsappReception,handleWhatsappOperatorCommand,requestJson,evolutionEndpoint,waitForEvolutionOpen,whatsappStatus,telegramStatus,webhookAuthStatus,incomingWhatsappMessage};
