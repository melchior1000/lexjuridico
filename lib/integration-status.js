...
async function publicWhatsappReception(body, instance, options = {}) {
  ...
  const marker = item.urgente ? '[URGENTE]' : '[RECEPÇÃO]';

  await _sendEvolutionText(operator, `${marker} ${name} (${sender}) — ${item.classe}: ${safeOriginal}`, instance, {url:cfg.url,key:cfg.key,request});
