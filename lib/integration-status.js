...
const {normalizeOwnerDeskCommand} = require('./owner-desk');
...
async function handleWhatsappOperatorCommand(body, instance, options = {}) {
  ...
  const text=/^\//.test(rawText)?rawText:naturalCommand;
  ...
  if(!operator||whatsappAccessMode(data.key?.remoteJid,operator)!=='operator'||data.key?.fromMe!==false||!text) return false;
...
async function publicWhatsappReception(body, instance, options = {}) {
  ...
  // Arquivamento é decisão explícita do dono, nunca consequência de uma frase.
  ...
  if(level==='urgente') await _sendEvolutionText(operator,summary,instance,{url:cfg.url,key:cfg.key,request});
  ...
  const reported=await _sendEvolutionText(operator,(level==='urgente'?'Contato '+contactName+' ('+sender+')':summary)+'\n'+outcome,instance,{url:cfg.url,key:cfg.key,request});
...
function incomingWhatsappMessage(body,instance){
  ...
  if(mode==='operator'){
    const operatorText=whatsappMessageText(data);
    const privateCommand=/^\/(recepcao|historico|responder|arquivar|ajuda)\b/i.test(operatorText)||!!normalizeOwnerDeskCommand(operatorText);
    if(privateCommand){queueMicrotask(()=>handleWhatsappOperatorCommand(body,instance).catch(()=>{}));return false;}
  }
  return true;
}
