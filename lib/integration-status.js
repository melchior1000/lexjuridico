...
function publicWhatsappReply(text) {
  const t = String(text || '').trim();
  const n = t.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
  if (/quem (e|eh)|de quem|esse numero|este numero/.test(n)) {
    return 'Olá! Este é o WhatsApp do Lex Jurídico. Posso anotar seu nome ou empresa e o motivo do contato para encaminhar ao responsável.';
  }
  if (/processo|acao|audiencia|peticao|recurso|sentenca|prazo|andamento|meu caso|advogad/.test(n)) {
    return 'Olá! Por segurança e sigilo, este canal não fornece informações processuais automaticamente. Posso registrar seu nome e o assunto para o responsável retornar.';
  }
  if (/cobranca|vivo|claro|tim|operadora|fornecedor|financeiro|boleto|fatura/.test(n)) {
    return 'Olá! Recebi sua mensagem. Informe, por favor, seu nome ou empresa e o assunto do contato. Vou encaminhar ao responsável.';
  }
  if (/^(oi|ola|bom dia|boa tarde|boa noite|opa|al[oô])\b/.test(n)) {
    return 'Olá! Você falou com o Lex Jurídico. Como posso ajudar? Informe seu nome ou empresa e, em poucas palavras, o assunto do contato.';
  }
  return 'Olá! Recebi sua mensagem no Lex Jurídico. Informe seu nome ou empresa e o assunto do contato para eu encaminhar ao responsável.';
}
...
function _publicInbox() {
  if (!global._whatsappPublicInbox) global._whatsappPublicInbox = [];
  return global._whatsappPublicInbox;
}

function _publicCategory(text) {
  const n = String(text || '').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
  if (/prisao|preso|mandado|liminar|audiencia (hoje|amanha)|prazo (hoje|amanha)|bloqueio urgente|urgente/.test(n)) return {category:'urgente', urgent:true};
  if (/processo|acao|audiencia|peticao|recurso|sentenca|prazo|andamento|advogad/.test(n)) return {category:'juridico', urgent:false};
  if (/cobranca|vivo|claro|tim|operadora|fornecedor|financeiro|boleto|fatura/.test(n)) return {category:'administrativo', urgent:false};
  return {category:'geral', urgent:false};
}

function _upsertPublicInbox(sender, name, text) {
  const inbox = _publicInbox();
  const now = new Date().toISOString();
  const kind = _publicCategory(text);
  let item = inbox.find(x => x.number === sender && x.status === 'aguardando_advogado');
  if (!item) {
    item = {id:'wa_'+Date.now()+'_'+Math.random().toString(36).slice(2,6), number:sender, name:name||'Contato', status:'aguardando_advogado', created_at:now, count:0};
    inbox.push(item);
  }
  item.name = name || item.name;
  item.last_text = String(text || '').substring(0,1000);
  item.last_at = now;
  item.count = (item.count || 0) + 1;
  item.category = kind.category;
  item.urgent = !!(item.urgent || kind.urgent);
  if (inbox.length > 200) inbox.splice(0, inbox.length - 200);
  return item;
}

function _listReceptionText() {
  const rows = _publicInbox().filter(x=>x.status==='aguardando_advogado')
    .sort((a,b)=>(Number(b.urgent)-Number(a.urgent)) || String(b.last_at).localeCompare(String(a.last_at))).slice(0,10);
  if (!rows.length) return 'Recepção: ninguém aguardando retorno.';
  return 'Recepção — aguardando você:\n' + rows.map((x,i)=>`${i+1}. ${x.urgent?'🔴 ':''}${x.name} (${x.number}) — ${x.category} — ${String(x.last_text||'').substring(0,120)}`).join('\n') + '\n\nUse /responder NUMERO mensagem ou /arquivar NUMERO.';
}
...
async function handleWhatsappOperatorCommand(body, instance, options = {}) {
  ...
  if (/^\/recepcao\s*$/i.test(text)) {
    await _sendEvolutionText(operator, _listReceptionText(), instance, {url:cfg.url,key:cfg.key,request});
    return true;
  }
  ...
  if (replyMatch) {
    ...
    const ok = await _sendEvolutionText(target, replyMatch[2].trim(), instance, {url:cfg.url,key:cfg.key,request});
  ...
  if (archiveMatch) {
    ...
    const item = target && _publicInbox().find(x=>x.number===target && x.status==='aguardando_advogado');
    if (item) { item.status='arquivado'; item.archived_at=new Date().toISOString(); }
    await _sendEvolutionText(operator, item ? `Contato ${target} arquivado.` : 'Contato não encontrado na fila.', instance, {url:cfg.url,key:cfg.key,request});
    return true;
  }
  return false;
}
...
async function publicWhatsappReception(body, instance, options = {}) {
  ...
  const original = whatsappMessageText(data) || '[mídia ou arquivo sem texto]';
  const item = _upsertPublicInbox(sender, name, original);
  const reply = publicWhatsappReply(original);
  const safeOriginal = original.replace(/[\r\n]+/g,' ').substring(0,1000);
  const marker = item.urgent ? '[URGENTE]' : '[RECEPÇÃO]';

  await _sendEvolutionText(operator, `${marker} ${name} (${sender}) — ${item.category}: ${safeOriginal}`, instance, {url:cfg.url,key:cfg.key,request});
  const replied = await _sendEvolutionText(sender, reply, instance, {url:cfg.url,key:cfg.key,request});
  await _sendEvolutionText(operator, `[LEX → ${sender}] ${reply}`, instance, {url:cfg.url,key:cfg.key,request});
  return replied;
}
...
function incomingWhatsappMessage(body, instance) {
  ...
  if (mode === 'operator' && /^\/(recepcao|responder|arquivar|ajuda)\b/i.test(whatsappMessageText(data))) {
...
module.exports = {
  brazilMobile, sameBrazilWhatsappNumber, whatsappAccessMode, publicWhatsappReply,
