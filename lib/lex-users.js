...
function publico(u){return u&&{id:u.id,nome:u.nome,email:u.email,papel:u.papel,oab:u.oab||null,ativo:u.ativo!==false,criado_em:u.criado_em||null,desativado_em:u.desativado_em||null}}
...
function createUserStore({load,save,now=()=>new Date()}){
  ...
  return{
    ...
    async criar({nome,email,papel,senha,oab}){
      ...
      const u={id:crypto.randomUUID(),nome:String(nome).trim().slice(0,120),email:e,papel,oab:oab?String(oab).trim().toUpperCase():null,senha_hash:hashSenha(senha),ativo:true,criado_em:now().toISOString()};
