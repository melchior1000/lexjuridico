...
function createUserStore({load,save,now=()=>new Date()}){
  let cache=null,opcoes={};
  async function lista(){if(!cache){const v=await load();cache=Array.isArray(v?.lista)?v.lista:[];opcoes=v?.opcoes||{}}return cache}
  async function gravar(next,novasOpcoes=opcoes){await save({lista:next,opcoes:novasOpcoes});cache=next;opcoes=novasOpcoes}
  return{
    ...
    async carregar(){const v=await load();if(Array.isArray(v?.lista)){cache=v.lista;opcoes=v.opcoes||{}}else if(!cache)cache=[];return cache},
    // Login compartilhado (perfil + senha única) desligado: só contas individuais entram.
    perfilDesligado(){return opcoes.login_compartilhado_desligado===true},
    async desligarLoginCompartilhado(desligar){
      const atual=await lista();
      if(desligar&&!atual.some(u=>u.ativo!==false&&u.senior===true))throw erro('Crie antes uma conta de advogado sênior ativa; senão ninguém mais consegue entrar para gerenciar.');
      await gravar(atual,{...opcoes,login_compartilhado_desligado:!!desligar});
      return this.perfilDesligado();
    },
    async trocarPropriaSenha(id,senhaAtual,senhaNova){
      const u=(await lista()).find(x=>x.id===id&&x.ativo!==false);
      if(!u||!conferirSenha(senhaAtual,u.senha_hash))throw erro('Senha atual incorreta.',401);
      return this.trocarSenha(id,senhaNova);
    },
