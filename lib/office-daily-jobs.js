...
async function runDailyOfficeJobs({processStore,sbReq,now=new Date(),datajudOptions={},djenOptions={}}={}){
  ...
  if(Array.isArray(djen.cunhar)&&djen.cunhar.length&&typeof sbReq==='function'){
    const suggested=await DjenMonitor.suggestPendingDeadlines(sbReq,djen.cunhar,{
      aiAnalyze:djenOptions.aiAnalyze,feriados:djenOptions.feriados||[],calendarioVerificado:djenOptions.calendarioVerificado===true
    });
    djen.cunhar=suggested.rows;
    djen.sugestao_falhas=suggested.failures;
  }
  ...
  return Object.freeze({
    ...
    djen:{
      ...
      cunhar_total:Array.isArray(djen.cunhar)?djen.cunhar.length:0,cunhar:Array.isArray(djen.cunhar)?djen.cunhar:[],sugestao_falhas:Array.isArray(djen.sugestao_falhas)?djen.sugestao_falhas:[]
    },
    datajud:{enabled:datajudEnabled,total:sync.total,novos:Number(sync.novos)||0,falhas:failed.length,skipped:sync.skipped===true,reason:sync.reason||null},
