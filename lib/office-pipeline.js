...
function normalizeChecklist(input={}){
  ...
  for(const [key,value] of Object.entries(input||{})){
    const k=norm(key).replace(/[^a-z0-9_]+/g,'_').replace(/^_+|_+$/g,'');
    const v=norm(value).replace(/\s+/g,'_');
    if(!k) continue;
    if(!CHECK_VALUES.has(v)) throw new Error('Checklist inválido em '+key+'. Use ok, falta ou nao_se_aplica.');
    out[k]=v;
  }
  return out;
}
function checklistStatus(p={}){
  const checklist=normalizeChecklist(p.checklist_cadastro||{});
  const keys=Object.keys(checklist);
  const pendentes=keys.filter(k=>checklist[k]==='falta');
  const faltando=missingDocs(p);
  const conferido=p.cadastro_conferido===true && keys.length>0 && pendentes.length===0 && !faltando;
  return {checklist,pendentes,faltando,conferido};
}
function stageOf(p={}){
...
function validateHandoff(current,to,{allowReopen=false}={}){
  ...
  if(from==='cadastro' && target==='iniciais'){
    const check=checklistStatus(current);
    if(!check.conferido) throw new Error(check.faltando?'Cadastro incompleto: existem documentos pendentes.':check.pendentes.length?'Cadastro incompleto: checklist possui pendências.':'Cadastro não conferido: confirme o checklist antes de encaminhar.');
  }
...
function handoff(current,to,meta={}){
  ...
  const patch={office_stage:target,fluxo_setor:target,setor_desde:ev.criado_em,office_events:[ev,...array(current.office_events)].slice(0,200),office_last_event:ev,atualizado_em:ev.criado_em};
  if(target==='cadastro' && from!=='recepcao') patch.ultima_devolucao={de:from,para:target,motivo:ev.motivo,em:ev.criado_em,por:ev.por};
  return {...current,...patch};
}
function confirmRegistration(current,checklist,meta={}){
  if(stageOf(current)!=='cadastro') throw new Error('A conferência de cadastro só pode ser feita no setor Cadastro.');
  const normalized=normalizeChecklist(checklist);
  if(!Object.keys(normalized).length) throw new Error('Informe o checklist de cadastro.');
  const pendentes=Object.entries(normalized).filter(([,v])=>v==='falta').map(([k])=>k);
  if(missingDocs(current) || pendentes.length){
    return {...current,checklist_cadastro:normalized,cadastro_conferido:false,bloqueio_peca:true,atualizado_em:meta.now||new Date().toISOString()};
  }
  return {...current,checklist_cadastro:normalized,cadastro_conferido:true,bloqueio_peca:false,cadastro_conferido_em:meta.now||new Date().toISOString(),cadastro_conferido_por:String(meta.actor||'LEX Cadastro'),atualizado_em:meta.now||new Date().toISOString()};
}
function taskStartStage(type,currentStage=null){
  const t=norm(type);
  ...
  if(t==='analise') return currentStage==='cadastro'?'iniciais':currentStage;
  return null;
}
...
async function confirmProcessRegistration(processStore,processId,checklist,actor='LEX Cadastro'){
  if(!processStore?.mutate) return null;
  const result=await processStore.mutate(ps=>{
    const i=ps.findIndex(p=>String(p.id)===String(processId));
    if(i<0) throw Object.assign(new Error('Processo não encontrado para conferência.'),{status:404});
    ps[i]=confirmRegistration(ps[i],checklist,{actor});
    return ps[i];
  },actor);
  return result.value;
}
async function syncTaskStart(processStore,task,actor='LEX'){
  ...
  const result=await processStore.mutate(ps=>{
    ...
    if(i<0) throw Object.assign(new Error('Processo não encontrado para iniciar tarefa.'),{status:404});
    let p=ps[i],from=stageOf(p);
    if(from==='recepcao') throw new Error('O caso ainda está na Recepção. Conclua o Cadastro antes da produção jurídica.');
    if(from==='cadastro') {
      const check=checklistStatus(p);
      if(!check.conferido) throw new Error('Produção bloqueada: o Cadastro precisa ser conferido e não pode ter documentos pendentes.');
    }
    let target=taskStartStage(task.tipo,from);
    if(from==='cadastro' && target==='iniciais') {
      p=handoff(p,'iniciais',{actor,agent:'LEX Cadastro',taskId:task.id,reason:'Cadastro conferido; documentação liberada para análise'});
      from='iniciais'; target=taskStartStage(task.tipo,from);
    } else if(from==='cadastro' && target && target!=='cadastro') {
    ...
    if(target && from!==target) p=handoff(p,target,{actor,agent:task.agente||'LEX',taskId:task.id,reason:'Tarefa '+task.tipo+' iniciada pelo LEX'});
    ps[i]=p;return p;
  },actor);
  return result.value;
}
...
module.exports={STAGES,ALLOWED,CHECK_VALUES,stageOf,summary,missingDocs,normalizeChecklist,checklistStatus,validateHandoff,handoff,confirmRegistration,taskStartStage,taskResultStage,postReviewStage,moveProcess,confirmProcessRegistration,syncTaskStart,syncTaskResult,syncTaskReview};
