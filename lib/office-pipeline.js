...
function confirmRegistration(current,checklist,meta={}){
  ...
  if(missingDocs(current) || pendentes.length){
    return {...current,checklist_cadastro:normalized,cadastro_conferido:false,bloqueio_peca:true,atualizado_em:now};
  }
  return {...current,checklist_cadastro:normalized,cadastro_conferido:true,bloqueio_peca:false,cadastro_conferido_em:now,cadastro_conferido_por:String(meta.actor||'LEX Cadastro'),atualizado_em:now};
}
...
async function confirmAndForwardRegistration(processStore,processId,checklist,actor='LEX Cadastro'){
  if(!processStore?.mutate) return null;
  const result=await processStore.mutate(ps=>{
    const i=ps.findIndex(p=>String(p.id)===String(processId));
    if(i<0) throw Object.assign(new Error('Processo não encontrado para conferência.'),{status:404});
    let p=confirmRegistration(ps[i],checklist,{actor});
    if(checklistStatus(p).conferido) p=handoff(p,'iniciais',{actor,agent:'LEX Cadastro',reason:'Cadastro conferido; documentação pronta para Iniciais'});
    ps[i]=p;return p;
  },actor);
  return result.value;
}
...
module.exports={STAGES,ALLOWED,CHECK_VALUES,stageOf,summary,missingDocs,normalizeChecklist,checklistStatus,validateHandoff,handoff,confirmRegistration,taskStartStage,taskResultStage,postReviewStage,moveProcess,confirmProcessRegistration,confirmAndForwardRegistration,syncTaskStart,syncTaskResult,syncTaskReview};
