...
async function dailyReceipts(deps,command,now){
  ...
  // 2) Atendimentos da recepção (WhatsApp) — eventos de saída do LEX hoje
  try{
    const store=deps.reception||deps.receptionStore;
    if(store&&typeof store.list==='function'){
      const items=await store.list({status:'aguardando_advogado',limit:50});
  ...
  try{
    ...
    for(const p of processes){
      ...
      for(const a of Array.isArray(p.andamentos)?p.andamentos:[]){
        ...
        if(src&&(stamp?dayOf(stamp)===today:dayOf(a.data)===today))add({tipo:'andamento_registrado',quando:stamp||null,oque:String(a.txt||'').slice(0,200),para:label,autorizado_por:null,origem:src,ref:p.id});
      }
