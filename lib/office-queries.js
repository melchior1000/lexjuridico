...
async function dailyReceipts(deps,command,now){
  ...
  try{
    ...
    if(log)add({tipo:'rotina_noturna',quando:log.terminou_em||log.iniciou_em,oque:'Atualizei a carteira: '+Number(log.consultados||0)+' consultados · '+Number(log.novos||0)+' andamentos novos · '+Number(log.erros||0)+' erros',para:'Datajud/DJEN',autorizado_por:null,origem:'datajud',ref:null});
  }catch{/* opcional */}
