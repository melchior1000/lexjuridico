...
const HELP=[
  ...
  '• "minha OAB é 123456/MG" (liga as publicações do Diário)',
  '• "teste o PJe" (diz se está conectado e, se não, o motivo)',
...
function oabMessage(r){
  ...
  const d=r.djen;
  if(!d)return'✅ OAB '+lista+' ligada ao LEX. Vou buscar as publicações do DJEN na próxima rodada.';
  ...
  const lines=['✅ OAB '+lista+' ligada ao LEX. Li o DJEN agora: '+(d.consultadas||0)+' publicação(ões).'];
...
async function executeOfficeQuery(deps,command,extra={}){
  ...
  if(command.action==='oab_set'){
    if(typeof deps.oab?.set!=='function')return{handled:true,command,message:'Não consegui ligar a OAB por aqui. Use a tela Mais → Ligar ao tribunal.'};
...
const QUERY_ACTIONS=Object.freeze(['help','deadlines','publications','process_status','daily_brief','pje_avisos','pje_open','pje_confirm','court_update','carteira_audit','cnj_fix','pje_teste','oab_set']);
