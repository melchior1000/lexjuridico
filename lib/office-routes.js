...
const {authorizationForAction,capabilityForAction,CAPABILITY_VERSION}=require('./agent-capabilities');
const {createExecutionReceipt}=require('./execution-receipt');
...
async function finalizeExecution(deps,input,command,outcome,evidence={}){
  const requestedAction=String(input.requested_action||command?.action||'unknown');
  const resolvedAction=String(command?.action||requestedAction);
  const capability=capabilityForAction(resolvedAction);
  const receipt=createExecutionReceipt({
    intent_id:input.request_id||null,
    action:requestedAction,
    resolved_action:resolvedAction,
    actor:String(input.profile||input.perfil||'').trim(),
    process_id:command?.processo_id??outcome?.result?.processo_id??outcome?.result?.processo?.id??null,
    effect:capability?.effect||null,
    capability_version:CAPABILITY_VERSION,
    request:{text:String(input.text||input.mensagem||''),processo_id:input.processo_id??null},
    result:outcome?.result??null,
    evidence,
    runtime_fingerprint:outcome?.result?.runtime?.fingerprint||null
  });
  let receiptPersisted=false;
  if(deps.records?.change){
    try{await deps.records.change('lex_execution_receipt_'+receipt.receipt_id,()=>receipt);receiptPersisted=true;}
    catch(e){deps.log?.('[execution-receipt] '+e.message);}
  }
  return{...outcome,receipt,receipt_persisted:receiptPersisted};
}
async function executeNaturalOfficeCommand(deps,input={}){
  ...
  const authorization=authorizationForAction(command.action,profile);
  if(!authorization.allowed)return finalizeExecution(deps,input,command,{handled:true,command,message:'Seu perfil não tem permissão para executar esta ordem.'},{blocked:true,reason:authorization.known?'permission_denied':'unknown_action'});
  const resolved=await resolveCommandProcess(deps,command,text);
  if(resolved.needs_input)return finalizeExecution(deps,input,command,{handled:true,command,needs_input:true,message:resolved.message,candidates:resolved.candidates||[]},{blocked:true,reason:'process_ambiguous_or_missing'});
  ...
  if(command.action==='task'||command.action==='lex_review'){
    ...
    if(input.defer_task===true){
      ...
      return finalizeExecution(deps,input,{...command,tipo},{handled:true,command:{...command,tipo},task,result:queued,message:'Ordem registrada para '+tipo+'. Tarefa '+String(task.id||'').slice(0,8)+' aceita na fila; a execução será confirmada pelo estado da tarefa.'},{task_id:task.id,queue_status:'na_fila',deferred:true,process_id:command.processo_id});
    }
    const result=await runTaskThroughOffice(deps,task,profile);
    return finalizeExecution(deps,input,{...command,tipo},{handled:true,command:{...command,tipo},task,result,message:taskMessage(task,result)},{task_id:task.id,result_status:result?.status||null,process_id:command.processo_id,runtime_fingerprint:result?.runtime?.fingerprint||null});
  }
  if(command.action==='send_previous'){
    const previous=String(input.previous_text||'').trim();
    if(!command.contato)return finalizeExecution(deps,input,command,{handled:true,command,needs_input:true,message:'Para qual contato devo enviar o texto anterior pelo '+command.canal+'?'},{blocked:true,reason:'contact_missing'});
    if(!previous)return finalizeExecution(deps,input,command,{handled:true,command,needs_input:true,message:'Não encontrei um texto anterior confirmado nesta conversa para enviar. Diga o texto exato.'},{blocked:true,reason:'previous_text_missing'});
    if(previous.length>3500)return finalizeExecution(deps,input,command,{handled:true,command,needs_input:true,message:'O texto anterior é longo demais para envio automático. Indique o trecho exato que devo mandar.'},{blocked:true,reason:'previous_text_too_long'});
    return executeNaturalOfficeCommand(deps,{...input,text:'Responda a '+command.contato+': '+previous,previous_text:null,force_channel:command.canal,requested_action:input.requested_action||'send_previous'});
  }
  if(command.action==='reply_contact'){
    ...
    if(matches.length!==1){
      ...
      return finalizeExecution(deps,input,command,{handled:true,command,needs_input:true,message,candidates:matches.slice(0,8).map(x=>({nome:x.nome,origem:x.origem,id:x.id}))},{blocked:true,reason:matches.length?'contact_ambiguous':'contact_not_found'});
    }
    const target=matches[0];
    if(!command.texto)return finalizeExecution(deps,input,command,{handled:true,command,needs_input:true,message:'Encontrei '+target.nome+' no '+target.origem+'. Diga o texto exato que devo enviar.',contact:{nome:target.nome,origem:target.origem,id:target.id}},{blocked:true,reason:'message_text_missing'});
    ...
    if(sent!==true)return finalizeExecution(deps,input,command,{handled:true,command,message:'O provedor não confirmou o envio para '+target.nome+'. Nada foi registrado como enviado.',sent:false},{failed:true,reason:'provider_not_confirmed',provider_confirmed:false,channel:target.origem,target_ref_hash:crypto.createHash('sha256').update(target.origem+':'+target.id).digest('hex')});
    const store=deps.receptionStore||receptionStore;
    ...
    try{
      ...
      else historyRecorded=false;
    }catch(e){historyRecorded=false;deps.log?.('[channel-history] '+e.message);}
    const result={enviado:true,nome:target.nome,origem:target.origem,id:target.id,historico_registrado:historyRecorded};
    const message=historyRecorded?commandResponse(command,result):'Mensagem enviada para '+target.nome+' pelo '+target.origem+', mas o histórico local não confirmou o registro. Não reenvie automaticamente; confira o canal.';
    return finalizeExecution(deps,input,command,{handled:true,command,result,message},{provider_confirmed:true,history_recorded:historyRecorded,channel:target.origem,target_ref_hash:crypto.createHash('sha256').update(target.origem+':'+target.id).digest('hex')});
  }
  if(command.action==='register_contact'){
    ...
    if(matches.length!==1){
      ...
      return finalizeExecution(deps,input,command,{handled:true,command,needs_input:true,message,candidates:(matches.length?matches:rows).slice(0,8).map(x=>({nome:x.nome,origem:x.origem,id:x.id}))},{blocked:true,reason:matches.length>1?'contact_ambiguous':'contact_not_selected'});
    }
    ...
    return finalizeExecution(deps,input,command,{handled:true,command,result,message:result.message},{persisted:true,process_id:saved.value?.id||null,stage:result.setor,source_ref_hash:crypto.createHash('sha256').update(sourceKey).digest('hex')});
  }
  if(command.action==='work_queue'){
    ...
    return finalizeExecution(deps,input,command,{handled:true,command,result:{tarefas:pendentes,recepcao,urgentes},message},{observed:true,task_count:pendentes.length,reception_count:recepcao.length});
  }
  if(command.action==='datajud'){
    ...
    return finalizeExecution(deps,input,command,{handled:true,command,result,message:commandResponse(command,result)},{observed:true,process_id:command.processo_id});
  }
  if(command.action==='move'){
    ...
    const result={processo,setor:Pipeline.stageOf(processo)};return finalizeExecution(deps,input,command,{handled:true,command,result,message:commandResponse(command,result)},{persisted:true,process_id:processo?.id||command.processo_id,stage:result.setor});
  }
  if(command.action==='distribute'){
    if(!command.numero)return finalizeExecution(deps,input,command,{handled:true,command,needs_input:true,message:'Informe o número CNJ ou protocolo já confirmado antes de registrar a distribuição.'},{blocked:true,reason:'distribution_number_missing'});
    ...
    const result={processo:saved.value,setor:Pipeline.stageOf(saved.value),numero:saved.value?.numero||command.numero};return finalizeExecution(deps,input,command,{handled:true,command,result,message:commandResponse(command,result)},{persisted:true,process_id:saved.value?.id||command.processo_id,stage:result.setor,number:result.numero});
  }
  if(command.action==='confirm_registration')return finalizeExecution(deps,input,command,{handled:true,command,needs_input:true,message:'Para liberar o Cadastro, confirme o checklist e os documentos na tela de Cadastro. O LEX não presume documento conferido.'},{blocked:true,reason:'checklist_confirmation_required'});
  if(command.action==='court_watch'){
    ...
    const result={items,itens:items};return finalizeExecution(deps,input,command,{handled:true,command,result,message:commandResponse(command,result)},{observed:true,item_count:items.length});
  }
  return null;
}
