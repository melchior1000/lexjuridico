...
async function runTaskThroughOffice(deps,task,profile){
  try{
    ...
    if(deps.engine?.block) await deps.engine.block(task.id,e.message).catch(()=>{});
    return null;
  }
}

async function currentPreflight(deps,task){
  const state=await deps.processStore.read();
  return Pipeline.preflightTask(state.processes,task);
}
...
async function officeRoutes(req,res,deps) {
  ...
  try {
    if(path==='/api/escritorio/recepcao' && req.method==='GET') {
      ...
      const result=await deps.processStore.mutate(ps=>{
        ...
        const prep={...c,...pessoa};
        const p={...(i>=0?ps[i]:{}),id:c.id,nome:String(c.nome),setor:'autuacao',status:c.status||'EM_PREP',office_stage:'cadastro',fluxo_setor:'cadastro',
          area:c.area||'',numero:c.numero||'',partes:c.partes||'',tribunal:c.tribunal||'',descricao:c.obs||c.descricao||'',
          docsFaltantes:c.faltando||'',checklist_cadastro:c.checklist_cadastro??c.checklistCadastro??(i>=0?ps[i].checklist_cadastro:null),
      ...
      const state=await deps.processStore.read();
      const casos=state.processes.map(p=>({id:p.id,nome:p.nome||p.partes||'Processo',numero:p.numero||'',setor:Pipeline.stageOf(p),pendencias:Pipeline.cadastroPendencies(p),ultimo_movimento:p.office_last_event||null}));
      ...
      json(200,{ok:true,contagens:{...Workflow.summary(state.processes),setores:Pipeline.summary(state.processes)},versao:state.version,
        ...
        atualizacao:'banco',ia_configurada:deps.aiAvailable(),versao_aplicacao:'2026.09.14-office-pipeline'});
    } else if(path==='/api/tarefas' && req.method==='POST') {
      const input=await deps.body(req);
      await currentPreflight(deps,input);
      ...
      await currentPreflight(deps,existing);
  ...
  }catch(e){json(e.status||422,{error:e.message,pendencias:e.pendencias||undefined});}
  return true;
}
module.exports={officeRoutes,runTaskThroughOffice,currentPreflight};
