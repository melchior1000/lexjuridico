...
async function officeRoutes(req,res,deps) {
  ...
  if(path==='/api/escritorio/oab') {
    if(req.method==='GET'){const c=deps.pje?.config||null;json(200,{ok:true,oabs:typeof deps.oab?.get==='function'?deps.oab.get():[],pje:{configurado:!!c?.configurado,tribunais:(c?.tribunais||[]).map(t=>t.sigla),faltando:c?.faltando||['PJE_MNI_TRIBUNAIS','PJE_MNI_CPF','PJE_MNI_SENHA']}});return true;}
