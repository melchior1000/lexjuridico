...
async function executeNaturalOfficeCommand(deps,input={}){
  ...
  if(command.action==='reply_contact'){
    ...
    if(sent!==true)return finalizeExecution(deps,input,command,{handled:true,command,message:'O provedor não confirmou o envio para '+target.nome+'. Nada foi registrado como enviado.',sent:false},{failed:true,reason:'provider_not_confirmed',provider_confirmed:false,channel:target.origem,target_id:target.id});
    const store=deps.receptionStore||receptionStore;
    ...
    return finalizeExecution(deps,input,command,{handled:true,command,result,message},{provider_confirmed:true,history_recorded:historyRecorded,channel:target.origem,target_id:target.id});
  }
  if(command.action==='register_contact'){
    ...
    return finalizeExecution(deps,input,command,{handled:true,command,result,message:result.message},{persisted:true,process_id:saved.value?.id||null,stage:result.setor,source_key:sourceKey});
  }
