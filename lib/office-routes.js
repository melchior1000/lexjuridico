...
async function officeRoutes(req,res,deps) {
  ...
  if(path==='/api/escritorio/distribuir') {
    ...
    try{
      ...
      const state=await deps.processStore.read();
      const current=state.processes.find(p=>String(p.id)===String(processoId));
      ...
      const fresh=await deps.processStore.read();
      json(200,{ok:true,processo,setor:Pipeline.stageOf(processo),numero:processo.numero||null,status:processo.status||null,setores:Pipeline.summary(fresh.processes)});
    }catch(e){json(e.status||422,{error:e.message});}
    return true;
  }
  const secretariaPodePreparar=profile==='secretaria'&&path==='/api/escritorio/preparacao'&&req.method==='POST';
  const equipePodeRecepcao=['admin','advogado','secretaria'].includes(profile)&&path.startsWith('/api/escritorio/recepcao');
  if(!['admin','advogado'].includes(profile) && !secretariaPodePreparar && !equipePodeRecepcao) {json(403,{error:'Acesso restrito ao responsável jurídico'});return true;}
  try {
    if(path==='/api/escritorio/recepcao' && req.method==='GET') {
      if(!['admin','advogado','secretaria'].includes(profile)) {json(403,{error:'Recepção restrita à equipe do escritório'});return true;}
      ...
      json(200,{ok:true,contagens:{...Workflow.summary(state.processes),setores:Pipeline.summary(state.processes)},versao:state.version,
        ...
        atualizacao:'banco',ia_configurada:deps.aiAvailable(),versao_aplicacao:'2026.09.14-office-distribution'});
