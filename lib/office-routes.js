...
async function officeRoutes(req,res,deps) {
  ...
  if(path==='/api/escritorio/prazos/cunhar') {
    if(typeof deps.sbReq!=='function') {json(503,{error:'Persistência de prazos indisponível'});return true;}
    if(req.method==='GET'){
      ...
      try{
        const items=await DjenMonitor.listPendingMint(deps.sbReq,Number(parsedUrl.searchParams.get('limit'))||100);
    ...
    if(req.method==='POST'){
      ...
      try{
        ...
        const result=await DeadlineAuthorization.confirmDjenDeadline({
          processStore:deps.processStore,sbReq:deps.sbReq,djenId:b?.djen_id,dueAt:b?.due_at,humanId:profile,
  ...
  try {
    if(path==='/api/escritorio/recepcao' && req.method==='GET') {
      ...
      json(200,{ok:true,contagens:{...Workflow.summary(state.processes),setores:Pipeline.summary(state.processes)},versao:state.version,
        ...
        atualizacao:'banco',ia_configurada:deps.aiAvailable(),versao_aplicacao:'2026.09.20-office-atomic'});
