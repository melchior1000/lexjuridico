...
async function officeRoutes(req,res,deps) {
  ...
  const secretariaPodeRecibos=profile==='secretaria'&&path==='/api/escritorio/recibos'&&req.method==='GET';
  if(!['admin','advogado'].includes(profile) && !secretariaPodePreparar && !secretariaPodeRecepcao && !secretariaPodeRecibos) {json(403,{error:'Acesso restrito ao responsável jurídico'});return true;}
  try {
    if(path==='/api/escritorio/recepcao' && req.method==='GET') {
      ...
      if(!['admin','advogado','secretaria'].includes(profile)) {json(403,{error:'Recibos restritos à equipe autorizada'});return true;}
