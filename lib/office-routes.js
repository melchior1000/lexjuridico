...
async function resolveCommandProcess(deps,command,text){
  ...
  const selected=command.processo_id!=null&&String(command.processo_id).trim()?command.processo_id:null;
  const selection=resolveCase(state.processes||[],{processo_id:selected,instrucao:String(text||'')});
