...
async function assertTaskGate(deps,input){
  ...
  const stage=Pipeline.stageOf(p),target=Pipeline.taskStartStage(input.tipo);
  ...
  if(stage!==target) Pipeline.validateHandoff(p,target);
}
  ...
  try {
    if(path==='/api/escritorio/recepcao' && req.method==='GET') {
      ...
      json(200,{ok:true,contagens:{...Workflow.summary(state.processes),setores:Pipeline.summary(state.processes)},versao:state.version,
        ...
        atualizacao:'banco',ia_configurada:deps.aiAvailable(),versao_aplicacao:'2026.09.14-office-atomic'});
