...
const {parseOfficeQuery,executeOfficeQuery,processHint,isLegalQuestion,QUERY_ACTIONS}=require('./office-queries');
...
function publicCommandJob(job){
  if(!job)return null;
  return {
    id:job.id,origem:job.origem,contato_id:job.contato_id,contato_nome:job.contato_nome,
    comando:job.comando,status:job.status,texto_final:job.texto_final||null,last_error:job.last_error||null,
    provider_id:job.provider_id||null,provider_state:job.provider_state||null,
    criado_em:job.criado_em||null,atualizado_em:job.atualizado_em||null,
    confirmado_em:job.confirmado_em||null,enviado_em:job.enviado_em||null,finalizado_em:job.finalizado_em||null
  };
}
...
async function resolveCommandProcess(deps,command,text){
  ...
  if(!selection.process){
    const official=p=>!!(p?.last_court_sync_at||p?.partes_verificadas_em||p?.cadastro_conferido==='tribunal'||p?.numero_verificado_fonte==='pje');
    const label=p=>{
      if(official(p))return p.nome_oficial||p.nome||p.partes||'Processo';
      const m=String(p?.numero||'').match(/\d{7}-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}/);
      return m?'Processo '+m[0]+' — dados a conferir':'Processo — dados a conferir';
    };
    const candidates=(selection.candidates||[]).map(p=>({id:p.id,nome:label(p),numero:p.numero}));
    // No WhatsApp/Telegram não há lista clicável: as opções vão no próprio texto.
    const options=candidates.length?'\n'+candidates.map((c,i)=>(i+1)+') '+c.nome+(c.numero&&!c.nome.includes(c.numero)?' — '+c.numero:'')).join('\n')+'\nResponda com o número da opção (1, 2…) ou o CNJ.':'';
    return{needs_input:true,choice:'processo',message:(selection.reason||'Informe o processo.')+options,candidates};
  }
...
async function executeNaturalOfficeCommand(deps,input={}){
  ...
  let command=parseOfficeCommand(text,{processo_id:input.processo_id});
  if(command?.action==='task'&&isLegalQuestion(text))return null;
  const isQuery=!command;
  if(isQuery){command=parseOfficeQuery(text);if(command&&input.processo_id!=null)command.processo_id=input.processo_id;}
  if(!command)return null;
  const profile=String(input.profile||input.perfil||'').trim();
  if(!profile)throw Object.assign(new Error('Perfil não informado para executar a ordem.'),{status:401});
  const permissions={
    ...
    ...Object.fromEntries(QUERY_ACTIONS.map(action=>[action,['admin','advogado','secretaria']])),
    pje_confirm:['admin','advogado'],cnj_fix:['admin','advogado'],oab_set:['admin','advogado']
  };
  ...
  const resolved=await resolveCommandProcess(deps,command,command.hint??(isQuery?processHint(text):text));
  if(resolved.needs_input)return{handled:true,command,needs_input:true,choice:resolved.choice||null,message:resolved.message,candidates:resolved.candidates||[]};
  command=resolved.command;
  if(isQuery)return executeOfficeQuery(deps,command,{
    now:input.now,profile,
    workQueue:()=>executeNaturalOfficeCommand(deps,{text:'veja o que precisa de mim',profile})
  });
  if(command.action==='task'||command.action==='lex_review'){
    const tipo=command.action==='lex_review'?'revisao':command.tipo;
    if(process.env.LEX_AI_NO_CREDIT==='1'){
      const nome={analise:'Análise processual',peticao:'Redação de peça',contestacao:'Redação de contestação',recurso:'Redação de recurso',pericia:'Perícia analítica',quesitos:'Quesitos',revisao:'Revisão com IA'}[tipo]||tipo;
      return{handled:true,command:{...command,tipo},result:{status:'pausado_sem_credito',processo_id:command.processo_id||null},
        message:nome+' está pausada porque o provedor de IA está sem crédito. Não criei uma tarefa destinada a falhar e não alterei o processo. O banco, prazos, cadastro, mensagens, fontes e movimentação entre setores continuam operacionais.'};
    }
...
// Iniciativa do LEX: assim que uma tarefa termina (minuta pronta, falha ou pendência de dado),
// o titular é avisado na hora pelo canal (deps.onTaskResult), sem esperar o resumo do dia.
function taskResultNotice(result){
  if(!result||!result.status)return null;
  const id=String(result.id||'').slice(0,8),caso=result.processo_nome?' — '+result.processo_nome:'';
  if(result.status==='aguardando_revisao')return 'Minuta pronta para sua revisão: '+String(result.tipo||'peça')+caso+' (tarefa '+id+'). Nada foi protocolado. Diga "revisar '+id+'" ou abra em Tarefas.';
  if(['aguardando_dados','aguardando_documento_nitido','aguardando_configuracao'].includes(result.status))return 'Parei a tarefa '+id+caso+': '+String(result.pendencia||'preciso de um dado seu.');
  if(result.status==='falhou')return 'A tarefa '+id+caso+' falhou: '+String(result.pendencia||'sem detalhe')+'. Posso tentar de novo quando você mandar.';
  return null;
}
async function runTaskThroughOffice(deps,task,profile){
  try{
    ...
    if(typeof deps.onTaskResult==='function'){const notice=taskResultNotice(result);if(notice){try{await deps.onTaskResult(notice,result)}catch(e){deps.log?.('[office-pipeline] aviso não entregue: '+e.message)}}}
    return result;
...
async function officeRoutes(req,res,deps) {
  ...
  if(path==='/api/escritorio/pje/avisos') {
    if(!['admin','advogado','secretaria'].includes(profile)) {json(403,{error:'Perfil sem permissão para consultar expedientes do PJe'});return true;}
    if(req.method!=='GET') {json(405,{error:'Método não permitido'});return true;}
    const pje=deps.pje;
    if(!pje?.config?.configurado){json(200,{ok:true,configurado:false,faltando:pje?.config?.faltando||[],erro:pje?.config?.erro||null,avisos:[]});return true;}
    try{const listed=await pje.list();json(200,{ok:true,configurado:true,tribunais:pje.config.tribunais.map(t=>t.sigla),...listed});}
    catch(e){json(e.status||502,{error:e.message});}
    return true;
  }
  if(path==='/api/escritorio/oab') {
    if(req.method==='GET'){const c=deps.pje?.config||null;json(200,{ok:true,oabs:typeof deps.oab?.get==='function'?deps.oab.get():[],pje:{configurado:!!c?.configurado,eproc:[...(c?.eproc||[])],tribunais:(c?.tribunais||[]).map(t=>t.sigla),faltando:c?.faltando||['PJE_MNI_TRIBUNAIS','PJE_MNI_CPF','PJE_MNI_SENHA']}});return true;}
    if(!['admin','advogado'].includes(profile)) {json(403,{error:'Só advogado ou administrador liga a OAB'});return true;}
    if(req.method!=='POST') {json(405,{error:'Método não permitido'});return true;}
    if(typeof deps.oab?.set!=='function'){json(503,{error:'Ligação de OAB indisponível nesta instalação'});return true;}
    try{
      const b=await deps.body(req);
      const oabs=require('./djen-monitor').parseOabInput(b?.oab||b?.oabs||'');
      if(!oabs.length){json(400,{error:'OAB inválida. Exemplo: 123456/MG'});return true;}
      const r=await deps.oab.set(oabs,{profile});
      const {oabMessage}=require('./office-queries');
      json(200,{ok:true,...r,mensagem:oabMessage(r)});
    }catch(e){json(e.status||500,{error:e.message});}
    return true;
  }
  if(path==='/api/escritorio/processos/conferencia') {
    if(!['admin','advogado','secretaria'].includes(profile)) {json(403,{error:'Perfil sem permissão'});return true;}
    if(req.method!=='GET') {json(405,{error:'Método não permitido'});return true;}
    try{
      const Audit=require('./carteira-audit');
      const state=await deps.processStore.read();
      const dbReq=typeof deps.sbReq==='function'?deps.sbReq:deps.records?.request;
      let orphans=[],aviso=null;
      try{orphans=await Audit.readOrphans(dbReq)}catch(e){aviso='Publicações do DJEN indisponíveis: '+e.message}
      const report=Audit.auditCarteira({processes:state.processes||[],orphans});
      json(200,{ok:true,...report,aviso,mensagem:Audit.auditMessage(report)});
    }catch(e){json(e.status||500,{error:e.message});}
    return true;
  }
  if(path==='/api/escritorio/processos/numero') {
    if(!['admin','advogado'].includes(profile)) {json(403,{error:'Só advogado ou administrador corrige número de processo'});return true;}
    if(req.method!=='POST') {json(405,{error:'Método não permitido'});return true;}
    try{
      const b=await deps.body(req);
      const {correctAndRefresh,correctionMessage}=require('./carteira-audit');
      const dbReq=typeof deps.sbReq==='function'?deps.sbReq:deps.records?.request;
      const out=await correctAndRefresh({processStore:deps.processStore,processId:b?.processo_id,numero:b?.numero,actor:profile,dbReq,pje:deps.pje});
      json(200,{ok:true,...out,mensagem:correctionMessage(out)});
    }catch(e){json(e.status||500,{error:e.message});}
    return true;
  }
  if(path==='/api/escritorio/pje/processos') {
    if(!['admin','advogado','secretaria'].includes(profile)) {json(403,{error:'Perfil sem permissão para atualizar processos'});return true;}
    if(req.method!=='POST') {json(405,{error:'Método não permitido'});return true;}
    const pje=deps.pje;
    if(!pje?.config?.configurado){json(503,{error:'PJe (MNI) não configurado',faltando:pje?.config?.faltando||[]});return true;}
    try{
      const b=await deps.body(req);
      const {syncProcessesFromPje,syncReportMessage}=require('./pje-process-sync');
      const report=await syncProcessesFromPje({client:pje.client,processStore:deps.processStore,processId:b?.processo_id??null,oabs:(()=>{try{return require('./djen-monitor').parseOabs()}catch{return[]}})()});
      json(200,{...report,ok:true,sem_falhas:report.ok,mensagem:syncReportMessage(report)});
    }catch(e){json(e.status||502,{error:e.message});}
    return true;
  }
  if(path==='/api/escritorio/pje/sincronizar') {
    if(!['admin','advogado'].includes(profile)) {json(403,{error:'Perfil sem permissão para sincronizar o PJe'});return true;}
    if(req.method!=='POST') {json(405,{error:'Método não permitido'});return true;}
    const pje=deps.pje;
    if(!pje?.config?.configurado){json(503,{error:'PJe (MNI) não configurado',faltando:pje?.config?.faltando||[]});return true;}
    try{
      const {syncPjeAvisos}=require('./pje-monitor');
      const result=await syncPjeAvisos({client:pje.client,records:deps.records,processStore:deps.processStore});
      json(result.ok?200:502,{ok:result.ok,tribunais:result.tribunais.map(t=>({sigla:t.sigla,ok:t.ok,erro:t.erro||null,novos:t.novos.length,pendentes:t.pendentes??null}))});
    }catch(e){json(e.status||502,{error:e.message});}
    return true;
  }
  if(path==='/api/escritorio/prazos/cumprido') {
    if(!['admin','advogado'].includes(profile)) {json(403,{error:'Só advogado ou administrador baixa prazo'});return true;}
    if(req.method!=='POST') {json(405,{error:'Método não permitido'});return true;}
    try{
      const b=await deps.body(req),processoId=String(b?.processo_id||''),acao=String(b?.acao||'marcar');
      if(!processoId) throw Object.assign(new Error('Informe o processo.'),{status:400});
      if(!['marcar','desfazer'].includes(acao)) throw Object.assign(new Error('Ação de prazo inválida.'),{status:400});
      const now=new Date(),iso=now.toISOString(),br=new Intl.DateTimeFormat('pt-BR',{timeZone:'America/Sao_Paulo'}).format(now);
      const saved=await deps.processStore.mutate(ps=>{
        const i=ps.findIndex(p=>String(p.id)===processoId);
        if(i<0)throw Object.assign(new Error('Processo não encontrado.'),{status:404});
        const p=ps[i],andamentos=Array.isArray(p.andamentos)?p.andamentos.slice():[];
        if(acao==='marcar'){
          if(p?.prazo_baixa?.ativa===true)return p; // idempotente
          const truth=p?.deadline_truth&&typeof p.deadline_truth==='object'&&p.deadline_truth.legal_truth===true?p.deadline_truth:null;
          const due=String(truth?.due_at||p.prazoReal||p.prazo||p.dataPrazo||p.next_action_due_at||'').trim();
          if(!due)throw Object.assign(new Error('Este processo não tem prazo confirmado para baixar.'),{status:409});
          const actionId=crypto.randomUUID();
          const previous={
            prazo:p.prazo??null,prazoReal:p.prazoReal??null,dataPrazo:p.dataPrazo??null,next_action_due_at:p.next_action_due_at??null
          };
          andamentos.unshift({data:iso.slice(0,10),txt:'[LEX] Prazo '+due+' marcado como cumprido por '+profile+'.',origem:'lex',lex_action_id:actionId});
          ps[i]={...p,prazo:'',prazoReal:'',dataPrazo:'',next_action_due_at:'',
            prazo_cumprido:due,prazo_cumprido_em:iso,prazo_cumprido_por:profile,
            deadline_truth_resolvido_em:truth?iso:p.deadline_truth_resolvido_em||null,
            prazo_baixa:{ativa:true,action_id:actionId,vencimento:due,marcado_em:iso,marcado_por:profile,anterior:previous},
            andamentos};
          return ps[i];
        }
        const baixa=p?.prazo_baixa;
        if(!baixa?.ativa)throw Object.assign(new Error('Não há baixa ativa deste prazo para desfazer.'),{status:409});
        const prev=baixa.anterior||{},actionId=baixa.action_id||null;
        andamentos.unshift({data:iso.slice(0,10),txt:'[LEX] Baixa do prazo '+String(baixa.vencimento||'')+' desfeita por '+profile+'.',origem:'lex',lex_undo_of:actionId});
        ps[i]={...p,
          prazo:prev.prazo??'',prazoReal:prev.prazoReal??'',dataPrazo:prev.dataPrazo??'',next_action_due_at:prev.next_action_due_at??'',
          prazo_cumprido:'',prazo_cumprido_em:'',prazo_cumprido_por:'',
          deadline_truth_resolvido_em:'',
          prazo_baixa:{...baixa,ativa:false,desfeito_em:iso,desfeito_por:profile},
          andamentos};
        return ps[i];
      },profile);
      json(200,{ok:true,acao,processo:saved.value,em:iso,data:br});
    }catch(e){json(e.status||422,{error:e.message});}
    return true;
  }
  ...
  try {
    if(path==='/api/escritorio/recepcao' && req.method==='GET') {
    ...
    } else if(path==='/api/escritorio/recepcao/comandos' && req.method==='GET') {
      if(!deps.channelOutbox) {json(503,{error:'Fila de comandos do LEX indisponível'});return true;}
      const origem=String(parsedUrl.searchParams.get('origem')||'').toLowerCase();
      const id=String(parsedUrl.searchParams.get('id')||'').replace(/\D/g,'');
      const jobs=await deps.channelOutbox.listForContact(origem,id);
      json(200,{ok:true,jobs:jobs.map(publicCommandJob)});
    } else if(path==='/api/escritorio/recepcao/comando' && req.method==='POST') {
      if(!['admin','advogado','secretaria'].includes(profile)) {json(403,{error:'Recepção restrita à equipe autorizada'});return true;}
      if(!deps.channelOutbox) {json(503,{error:'Fila de comandos do LEX indisponível'});return true;}
      const body=await deps.body(req),origem=String(body?.origem||'').toLowerCase();
      const id=String(body?.id||body?.numero||'').replace(/\D/g,''),comando=String(body?.comando||'').trim();
      if(!['whatsapp','telegram'].includes(origem)||!id) throw new Error('Informe canal e contato.');
      if(!comando||comando.length>3500) throw new Error('Dê uma ordem ao LEX de até 3500 caracteres.');
      const rows=await activeUnifiedReception(deps);
      const target=rows.find(row=>row.origem===origem&&String(row.id)===id);
      if(!target) throw Object.assign(new Error('Contato não encontrado na Recepção ativa.'),{status:404});
      let historico=[];
      if(origem==='whatsapp') historico=await (deps.receptionStore||receptionStore).history(id,{limit:20});
      else historico=((await deps.records.read('lex_recepcao_telegram_'+id))?.value?.history||[]).slice(-20);
      const job=await deps.channelOutbox.enqueue({
        origem,id,nome:target.nome,comando,historico,profile,
        request_id:body?.request_id||req.headers?.['x-request-id']||undefined
      });
      json(202,{ok:true,job:publicCommandJob(job)});
    } else if(/^\/api\/escritorio\/recepcao\/comando\/[^/]+$/.test(path) && req.method==='GET') {
      if(!deps.channelOutbox) {json(503,{error:'Fila de comandos do LEX indisponível'});return true;}
      const jobId=decodeURIComponent(path.split('/').pop());
      const job=await deps.channelOutbox.read(jobId);
      if(!job) {json(404,{error:'Ordem não encontrada'});return true;}
      json(200,{ok:true,job:publicCommandJob(job)});
    } else if(/^\/api\/escritorio\/recepcao\/comando\/[^/]+\/confirmar$/.test(path) && req.method==='POST') {
      if(!deps.channelOutbox) {json(503,{error:'Fila de comandos do LEX indisponível'});return true;}
      const parts=path.split('/'),jobId=decodeURIComponent(parts[parts.length-2]),body=await deps.body(req);
      const job=await deps.channelOutbox.confirm(jobId,{texto:body?.texto});
      json(202,{ok:true,job:publicCommandJob(job)});
    } else if(/^\/api\/escritorio\/recepcao\/comando\/[^/]+\/cancelar$/.test(path) && req.method==='POST') {
      if(!deps.channelOutbox) {json(503,{error:'Fila de comandos do LEX indisponível'});return true;}
      const parts=path.split('/'),jobId=decodeURIComponent(parts[parts.length-2]);
      const job=await deps.channelOutbox.cancel(jobId);
      json(200,{ok:true,job:publicCommandJob(job)});
    ...
    } else if(path==='/api/escritorio/recibos' && req.method==='GET') {
      // Recibos do dia: só registros gravados (outbox, recepção, andamentos, prazos, tarefas).
      const receptionStore=deps.reception||deps.receptionStore||(()=>{try{return require('./whatsapp-reception-store').createReceptionStore()}catch{return null}})();
      const r=await executeOfficeQuery({...deps,reception:receptionStore},{action:'daily_receipts'},{now:new Date(),profile});
      json(200,{ok:true,...r.result,mensagem:r.message});
      ...
      const iaConfigurada=deps.aiAvailable();
      const iaEstado=process.env.LEX_AI_NO_CREDIT==='1'?'sem_credito':iaConfigurada?'disponivel':'sem_chave';
      json(200,{ok:true,contagens:{...Workflow.summary(state.processes),setores:Pipeline.summary(state.processes)},versao:state.version,
        ...
        atualizacao:'banco',ia_configurada:iaConfigurada,ia_estado:iaEstado,
        capacidades_operacionais:['recepcao','cadastro','processos','prazos','movimentacao','comunicacao'],
        capacidades_ia:['analise','redacao','jurisprudencia','pericia_analitica'],
        versao_aplicacao:'2026.09.24-concierge'});
...
module.exports={officeRoutes,runTaskThroughOffice,taskResultNotice,assertTaskGate,executeNaturalOfficeCommand,activeUnifiedReception};
