...
async function officeRoutes(req,res,deps) {
  ...
  try {
    if(path==='/api/escritorio/recepcao' && req.method==='GET') {
      ...
      const state=await deps.processStore.read();
      const casos=state.processes.map(p=>({
        id:p.id,nome:p.nome||p.partes||'Processo',numero:p.numero||'',setor:Pipeline.stageOf(p),
        cadastro:Pipeline.checklistStatus(p),ultimo_movimento:p.office_last_event||null
      }));
      ...
      json(200,{ok:true,contagens:{...Workflow.summary(state.processes),setores:Pipeline.summary(state.processes)},versao:state.version,
        ...
        atualizacao:'banco',ia_configurada:deps.aiAvailable(),versao_aplicacao:'2026.09.14-office-hardening'});
