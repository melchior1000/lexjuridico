...
async function executeNaturalOfficeCommand(deps,input={}){
  ...
  const permissions={
    ...
    reply_contact:['admin','advogado','secretaria'],work_queue:['admin','advogado','secretaria'],
  ...
  if(command.action==='reply_contact'){
    const rows=await activeUnifiedReception(deps),matches=receptionCandidates(rows,command.contato);
    if(matches.length!==1){
