...
async function officeRoutes(req,res,deps) {
  ...
  if(path==='/api/escritorio/processos/numero') {
    ...
    try{
      const b=await deps.body(req);
      const {applyCnjCorrection}=require('./carteira-audit');
      const out=await applyCnjCorrection({processStore:deps.processStore,processId:b?.processo_id,numero:b?.numero,actor:profile});
      json(200,{ok:true,processo:out.processo,anterior:out.anterior});
    }catch(e){json(e.status||500,{error:e.message});}
    return true;
  }
