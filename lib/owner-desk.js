...
function normalizeOwnerDeskCommand(value) {
  ...
  if (/^(mesa|resumo|recepcao)$/.test(simple)) {
    return '/recepcao';
  }
...
function normalizeOwnerNamedReply(value) {
  ...
  const match = text.match(/^(?:(?:a|ao)\s+)?([A-Za-zÀ-ÖØ-öø-ÿ][A-Za-zÀ-ÖØ-öø-ÿ' -]{1,79}?)\s*[,;:-]?\s*(?:diga|diz|avise|avisa|mande|manda|envie|envia|responda|responde)(?:\s+(?:a|pra|para)\s+el[ae])?\s+(?:q(?:ue)?\s+)?([\s\S]+)$/i);
