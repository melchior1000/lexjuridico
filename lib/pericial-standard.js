...
function memorialChecks(text){
  const t=norm(text);
  return {
    fontes:/(fonte|documento|folha|id)\b/.test(t),
    criterios:/(criterio|indice|taxa|marco temporal|base de correcao)/.test(t),
    metodologia:/(formula|ordem das operacoes|passo a passo|metodologia)/.test(t),
    consistencia:/(prova de consistencia|conferencia|batimento|teste de fechamento)/.test(t)
  };
}
