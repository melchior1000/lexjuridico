...
function memorialSection(text){
  const normalized=norm(text);
  const startMatch=/(memorial|memoria) de calculo/.exec(normalized);
  if(!startMatch) return '';
  const tail=normalized.slice(startMatch.index+startMatch[0].length);
  const next=/\b(respostas? aos quesitos|conclusao|anexos?)\b/.exec(tail);
  return (next?tail.slice(0,next.index):tail).trim();
}
function memorialChecks(text){
  const t=memorialSection(text);
  return {
...
function validatePericialDeliverable(text,{requireSources=true}={}){
  ...
  const sections=sectionChecks(body), memorial=memorialChecks(body), ids=sourceIds(body), money=monetaryClaims(body), problems=[];
...
module.exports={norm,sectionChecks,memorialSection,memorialChecks,sourceIds,monetaryClaims,validatePericialDeliverable,pericialSystemRules,auditPericialLabel};