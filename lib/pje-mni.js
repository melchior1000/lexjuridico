...
function mniConfig(env=process.env){
  ...
  // eproc pelo MNI do próprio eproc (PJE_MNI_EPROC="TRF6=https://…/controlador_ws.php?srv=intercomunicacao2.2"),
  // com credencial própria (PJE_MNI_EPROC_CPF/SENHA) ou, sem ela, a mesma do PJe.
  let eprocMni=[];
  ...
  const eprocCred={cpf:digits(env.PJE_MNI_EPROC_CPF||env.PJE_MNI_CPF),senha:String(env.PJE_MNI_EPROC_SENHA||env.PJE_MNI_SENHA||'')};
  const pjeTribunais=tribunais.length;
  tribunais=[...tribunais,...eprocMni.map(t=>({...t,credenciais:Object.freeze({...eprocCred,pfx:null,pfxSenha:''})}))];
  ...
  if(eprocMni.length){
    if(eprocCred.cpf.length!==11)faltando.push('PJE_MNI_EPROC_CPF');
...
async function diagnoseMni(client,config){
  ...
  for(const sigla of client.tribunais()){
    ...
    catch(e){tribunais.push({sigla,sistema:sistemas[sigla]||'pje',ok:false,codigo:e.code||'erro',erro:e.message,causa:CAUSA[e.code]||'erro inesperado: '+e.message,ms:Date.now()-inicio})}
  }
  return{ok:tribunais.length>0&&tribunais.every(t=>t.ok),configurado:true,tribunais,eproc};
}
function diagnoseMessage(d){
  const eprocLinha=(d.eproc||[]).length?'\n• '+d.eproc.join(', ')+': sistema eproc, não PJe — fora desta consulta. Para ligar, configure o webservice do eproc em PJE_MNI_EPROC (enquanto isso, publicações chegam pelo Diário).':'';
...
function createMniClient(config=mniConfig(),{transport,now=()=>new Date()}={}){
  ...
  async function call(sigla,operation,fields,parse){
    ...
    const {status,body}=await send(t.endpoint,envelope(operation,[...auth(t),...fields],config.ns));
