...
function parseTribunais(value,{sistema='pje',manterConsulta=false}={}){
  const out=[];
  for(const part of String(value||'').split(/[;\n]/).map(x=>x.trim()).filter(Boolean)){
    ...
    // eproc usa ?srv=intercomunicacao2.2 no endereço do webservice.
    if(!manterConsulta)url.search='';
    url.hash='';
    out.push({sigla:m[1].toUpperCase(),endpoint:url.toString(),sistema});
  }
  const seen=new Set();
...
function mniConfig(env=process.env){
  ...
  // eproc pelo MNI institucional. No TRF6, o próprio tribunal informa que a
  // integração exige credenciamento prévio e fornece usuário/senha próprios.
  // Login pessoal do eproc (CPF/OAB/senha/2FA) NÃO é credencial MNI.
  let eprocMni=[];
  try{eprocMni=parseTribunais(env.PJE_MNI_EPROC,{sistema:'eproc',manterConsulta:true})}catch(e){erro=erro||e.message.replace('PJE_MNI_TRIBUNAIS','PJE_MNI_EPROC')}
  const eprocSiglas=new Set(eprocMni.map(t=>t.sigla));
  const eproc=tribunais.filter(t=>isEproc(t.sigla,env)&&!eprocSiglas.has(t.sigla)).map(t=>t.sigla);
  tribunais=tribunais.filter(t=>!isEproc(t.sigla,env)&&!eprocSiglas.has(t.sigla));
  const eprocCred={
    // "cpf" é o nome interno legado do campo idConsultante. Para eproc/MNI
    // ele pode ser um usuário institucional não numérico.
    cpf:String(env.PJE_MNI_EPROC_USUARIO||'').trim(),
    senha:String(env.PJE_MNI_EPROC_SENHA||'')
  };
  const eprocNs={
    servico:String(env.PJE_MNI_EPROC_NS_SERVICO||'').trim(),
    tipos:String(env.PJE_MNI_EPROC_NS_TIPOS||'').trim()
  };
  const pjeTribunais=tribunais.length;
  tribunais=[...tribunais,...eprocMni.map(t=>({...t,
    credenciais:Object.freeze({...eprocCred,pfx:null,pfxSenha:''}),
    ns:Object.freeze({soap:NS.soap,...eprocNs})
  }))];
  const pfx=env.PJE_MNI_PFX_BASE64?Buffer.from(String(env.PJE_MNI_PFX_BASE64),'base64'):null;
  const faltando=[];
  if(pjeTribunais||!eprocMni.length){
    if(cpf.length!==11)faltando.push('PJE_MNI_CPF');
    if(!senha&&!pfx)faltando.push('PJE_MNI_SENHA');
  }
  if(eprocMni.length){
    if(!eprocCred.cpf)faltando.push('PJE_MNI_EPROC_USUARIO');
    if(!eprocCred.senha)faltando.push('PJE_MNI_EPROC_SENHA');
    // TRF6 informa MNI 2.2.3; não adivinhar namespace/WSDL. O valor deve vir
    // da documentação/credenciamento do tribunal.
    if(!eprocNs.servico)faltando.push('PJE_MNI_EPROC_NS_SERVICO');
    if(!eprocNs.tipos)faltando.push('PJE_MNI_EPROC_NS_TIPOS');
  }
...
async function diagnoseMni(client,config){
  ...
  const tribunais=[];
  const sistemas=Object.fromEntries((config.tribunais||[]).map(t=>[t.sigla,t.sistema||'pje']));
  for(const sigla of client.tribunais()){
    const inicio=Date.now();
    try{const r=await client.consultarAvisosPendentes(sigla);tribunais.push({sigla,sistema:sistemas[sigla]||'pje',ok:true,avisos:Array.isArray(r)?r.length:Array.isArray(r?.avisos)?r.avisos.length:0,ms:Date.now()-inicio})}
    catch(e){
      const sistema=sistemas[sigla]||'pje';
      const causa=sistema==='eproc'&&e.code==='autenticacao'
        ?'o tribunal recusou a credencial MNI. Use o usuário/senha de integração fornecidos no credenciamento institucional; login pessoal do eproc não substitui essa credencial.'
        :(CAUSA[e.code]||'erro inesperado: '+e.message);
      tribunais.push({sigla,sistema,ok:false,codigo:e.code||'erro',erro:e.message,causa,ms:Date.now()-inicio})
    }
  }
  return{ok:tribunais.length>0&&tribunais.every(t=>t.ok),configurado:true,tribunais,eproc};
}
function diagnoseMessage(d){
  const eprocLinha=(d.eproc||[]).length?'\n• '+d.eproc.join(', ')+': sistema eproc, não PJe — fora desta consulta. No TRF6, MNI exige credenciamento institucional e credencial própria fornecida pelo tribunal; login pessoal do eproc não basta. Enquanto isso, use consulta pública/DJEN e não trate o processo como conferido.':'';
  if(!d.configurado)return'A integração judicial ainda não está completa: '+(d.erro||'faltam '+d.faltando.join(', '))+'. Configure somente credenciais oficiais da integração e peça "teste o PJe" de novo.'+eprocLinha;
  ...
  for(const t of d.tribunais)lines.push(t.ok?'• '+t.sigla+(t.sistema==='eproc'?' (eproc)':'')+': conectado ('+t.avisos+' expediente(s) pendente(s)).':'• '+t.sigla+(t.sistema==='eproc'?' (eproc)':'')+': não conectou — '+t.causa+(t.erro&&!t.causa.includes(t.erro)?' ('+t.erro+')':''));
...
function createMniClient(config=mniConfig(),{transport,now=()=>new Date()}={}){
  ...
  // Cada tribunal pode ter credencial própria (eproc).
  const auth=t=>{const c=t?.credenciais||config.credenciais;return[['idConsultante',c.cpf],['senhaConsultante',c.senha]]};
  ...
  async function call(sigla,operation,fields,parse){
    ...
    const ns=t.ns?.servico&&t.ns?.tipos?t.ns:config.ns;
    const {status,body}=await send(t.endpoint,envelope(operation,[...auth(t),...fields],ns));
