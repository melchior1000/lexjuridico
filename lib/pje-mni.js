...
async function diagnoseMni(client,config){
  if(!config?.configurado)return{ok:false,configurado:false,faltando:config?.faltando||[],erro:config?.erro||null,tribunais:[]};
  const tribunais=[];
  ...
  return{ok:tribunais.length>0&&tribunais.every(t=>t.ok),configurado:true,tribunais};
}
function diagnoseMessage(d){
  if(!d.configurado)return'O PJe não está ligado ao LEX: '+(d.erro||'faltam '+d.faltando.join(', '))+'. Coloque esses dados no Render e peça "teste o PJe" de novo.';
...
module.exports={diagnoseMni,diagnoseMessage,NS,TIPO_COMUNICACAO,MniError,mniConfig,parseTribunais,envelope,mniDate,parseMniDate,parseAvisos,parseProcesso,parseTeor,createMniClient,soapTransport,teorConfirmationPhrase,assertTeorAuthorization,digits};
