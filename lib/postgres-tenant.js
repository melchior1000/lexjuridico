...
const {TENANT_TABLES,TENANT_CONFLICTS,normalizeTenantId,assertTableIsolated}=require('./supabase');
...
function tenantizeInput(method,table,data,query,tenantId){
  ...
  assertTableIsolated(table,true);
  const tenantScoped=TENANT_TABLES.has(table);
  const q={...(query||{})};
  let payload=data;
