...
const {cnjDigits,hasCnj}=require('./pje-sync');
...
function parseDate(value){
  ...
  const iso=s.match(/^(\d{4})-(\d{2})-(\d{2})(?:[T\s](\d{1,2}):(\d{2})(?::(\d{2})(\.\d{1,3})?)?(Z|[+-]\d{2}:?\d{2})?)?$/);
  if(!br&&!iso) return null;
  const parts=br?[br[3],br[2],br[1],br[4],br[5],br[6]]:iso.slice(1,7);
  const [y,mo,d,h=0,mi=0,sec=0]=parts.map((v,i)=>v===undefined&&i>=3?0:Number(v));
  if(y<1000||mo<1||mo>12||d<1||h>23||mi>59||sec>59) return null;
  const dt=new Date(Date.UTC(y,mo-1,d,h,mi,sec));
  if(dt.getUTCFullYear()!==y||dt.getUTCMonth()!==mo-1||dt.getUTCDate()!==d) return null;
  if(iso?.[7]) dt.setUTCMilliseconds(Number(iso[7])*1000);
  const zone=iso?.[8];
  if(zone&&zone!=='Z'){
    const digits=zone.replace(':','');
    const zh=Number(digits.slice(1,3)),zm=Number(digits.slice(3,5));
    if(zh>23||zm>59) return null;
    dt.setTime(dt.getTime()-(zone[0]==='+'?1:-1)*(zh*60+zm)*60000);
  }
  return dt;
}
...
function findProcess(processes,input){
  const numbers=[cnjDigits(input.numero_processo),cnjFromText(input.nome),cnjFromText(input.evento_texto)].filter(Boolean);
  if(new Set(numbers).size>1) throw new Error('Números de processo divergentes. Confira o documento e o destino.');
  const cnj=numbers[0];
  if(input.processo_id!=null && String(input.processo_id).trim()){
    ...
    if(!matches.length) throw new Error('O processo selecionado não existe.');
    if(cnj && cnjDigits(matches[0].numero)!==cnj) throw new Error('Número do documento diverge do processo selecionado.');
    return matches[0];
  }
  if(!cnj) return null;
  const matches=processes.filter(p=>hasCnj(p,cnj));
