...
async function registerProcess({cnj:input,processStore,pje=null,dbReq=null,now=new Date(),forcar=false,markCommunication=null}){
  ...
  if(isEproc(sigla))fonteTribunal='eproc';
  else if(pje?.config?.configurado&&pje.client&&pje.client.tribunais().includes(sigla)){
...
function registerMessage(r,cnj){
  ...
  const trib={pje:'',eproc:r.sigla+' usa eproc: o LEX ainda não lê o eproc; ',nao_conectado:r.sigla+' não está conectado ao LEX; ',falha:'O tribunal não respondeu ('+(r.erroTribunal||'erro')+'); '}[r.fonteTribunal]||'';
  ...
  const fontes=[r.fonteTribunal==='pje'?'tribunal (PJe)':null,r.publicacoes?r.publicacoes+' publicação(ões) do Diário':null].filter(Boolean);
