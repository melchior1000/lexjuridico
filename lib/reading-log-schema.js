...
function assertIJson(value,path='$'){
  ...
  if(value!==null&&typeof value==='object'&&Object.getOwnPropertySymbols(value).length)throw new Error('READING_IJSON_INVALID:'+path);
  if(typeof value==='number'){if(!Number.isFinite(value))throw new Error('READING_IJSON_INVALID:'+path);return}
  if(Array.isArray(value)){
    for(let i=0;i<value.length;i++){
      if(!(i in value)||value[i]===undefined)throw new Error('READING_IJSON_INVALID:'+path+'['+i+']');
      assertIJson(value[i],path+'['+i+']');
    }
    return;
  }
  if(typeof value==='object'){
    const prototype=Object.getPrototypeOf(value);
    if(prototype!==Object.prototype&&prototype!==null)throw new Error('READING_IJSON_INVALID:'+path);
    for(const key of Object.keys(value)){
      if(value[key]===undefined)throw new Error('READING_IJSON_INVALID:'+path+'.'+key);
...
function createReadingLogEntry(input={},opts={}){
  ...
  const queryInput=input.query_context&&typeof input.query_context==='object'?input.query_context:{};
  const metadataInput=input.metadata&&typeof input.metadata==='object'?input.metadata:{};
  assertIJson(queryInput,'$.query_context');assertIJson(metadataInput,'$.metadata');
  const queryContext=structuredClone(queryInput),metadata=structuredClone(metadataInput);
