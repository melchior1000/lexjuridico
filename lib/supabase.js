...
// Transição: as tabelas isoladas em 24/09/2026 só passam a receber
// escritorio_id depois que a migração 20260924120000 for aplicada e
// LEX_TENANCY_V2=1 for definido. Até lá se comportam como antes, para a
// publicação do código não quebrar o banco atual. O modo comercial exige V2.
const TENANCY_V2_TABLES = new Set(['arquivo_morto_indice','auditoria','cobrancas_pix','contatos','documentos_indexados','documentos_processo','memoria_checkpoints','mensagens_chat','perfis_juizes','tempo_uso','vivo_acoes','whatsapp_sessoes']);
function activeTenantTables(env = process.env) {
  if (env.LEX_TENANCY_V2 === '1') return TENANT_TABLES;
  return new Set([...TENANT_TABLES].filter(t => !TENANCY_V2_TABLES.has(t)));
}
...
function assertTableIsolated(table, tenantActive, tenantTables = TENANT_TABLES) {
  ...
  if (!set.has(table) && !GLOBAL_TABLES.has(table) && !TENANCY_V2_TABLES.has(table)) {
...
function createSupabaseRequest({
  ...
  tenantTables = activeTenantTables(),
