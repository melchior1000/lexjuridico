...
function sourceContext(p) {
  ...
  for(const d of [...flow.array(p.documentos),...flow.array(p.arquivos)]) {
    ...
    if(text) sources.push({
      id:'D'+(sources.length+1),origem:d.nome||'Documento anexado',texto:String(text).slice(0,30000),
      confianca_extracao:d.confianca_extracao||d.confianca||d.meta?.confianca_extracao||null,
      pagina:d.pagina||d.page||d.meta?.pagina||null,
      data:d.data||d.date||d.meta?.data||null,
      linha:d.linha||d.line||d.meta?.linha||null
    });
  }
...
function processSafetyContext(p) {
  ...
  const latest=movements.slice(-5).map(a=>({data:a.data||null,origem:a.origem||null,texto:String(a.texto||a.txt||a.descricao||'').slice(0,1500)}));
...
class TaskEngine {
  ...
  async run(id) {
    let claimed=false;
    const task=await this.store.change('lex_task:'+id,t=>{claimed=false;if(!t || t.status!=='na_fila') return undefined;claimed=true;return {...t,status:'executando',iniciada_em:new Date().toISOString(),tentativas:t.tentativas+1};});
    ...
    try {
      ...
      if(task.tipo==='pericia' || task.tipo==='quesitos') {
        const evidence=guardPericialDocuments(pericialDocuments(process),{requireCritical:true});
        if(!evidence.ok) return finish({
          status:'aguardando_documento_nitido',processo_id:process.id,
          controle_pericial:evidence,
          pendencia:'Perícia bloqueada: documento crítico sem leitura confiável. Envie arquivo nítido ou PDF original. Nenhum cálculo foi realizado.',
          aviso_assessor:auditLabel(evidence)
        });
      }
