...
class TaskEngine {
  ...
  async block(id,reason) {return this.store.change('lex_task:'+id,t=>{if(!t || !['na_fila','executando'].includes(t.status)) return undefined;return {...t,status:'aguardando_dados',pendencia:String(reason||'Fluxo do escritório bloqueou a tarefa.').slice(0,400),atualizada_em:new Date().toISOString()};});}
  async run(id) {
