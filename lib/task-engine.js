...
// Número citado em "A confirmar — vinculado a X" não identifica o processo X.
...
function resolveCase(processes,input) {
  ...
  if(explicit.length===1) {
    if(numbers.length && !hasCnj(explicit[0],numbers[0])) return {reason:'O número mencionado não corresponde ao processo selecionado.',candidates:explicit};
    return {process:explicit[0]};
  }
  if(input.processo_id!=null) return {reason:'O processo selecionado não existe.',candidates:[]};
  let candidates=numbers.length?processes.filter(p=>hasCnj(p,numbers[0])):processes.filter(p=>norm(p.nome).length>3 && norm(text).includes(norm(p.nome)));
