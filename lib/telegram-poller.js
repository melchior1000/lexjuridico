...
function createTelegramPoller({
  ...
  let failures=0, lastUpdateId=0, cursorLoaded=false, generation=0;
  ...
  async function ensurePollingMode(gen) {
    const info=await requestJson(api('getWebhookInfo'),{timeoutMs:10000});
    if(!info?.ok) throw new Error('getWebhookInfo nao confirmado');
    if(!String(info.result?.url||'').trim()) return false;
    if(!alive(gen)) return true;
    logger.warn?.('[Telegram] webhook legado detectado; removendo para ativar polling exclusivo.');
    const removed=await requestJson(api('deleteWebhook')+'?drop_pending_updates=false',{timeoutMs:10000});
    if(!removed?.ok) throw new Error('deleteWebhook nao confirmado');
    if(!alive(gen)) return true;
    const verify=await requestJson(api('getWebhookInfo'),{timeoutMs:10000});
    if(!verify?.ok || String(verify.result?.url||'').trim()) throw new Error('webhook permaneceu ativo apos deleteWebhook');
    logger.warn?.('[Telegram] webhook legado removido; polling exclusivo ativado.');
    return true;
  }
  ...
  async function tick() {
    ...
    try {
      ...
      if(is409) logger.warn?.('[Telegram] 409 Conflict: outro consumidor pode estar ativo; revalidando com backoff.');
  ...
  async function start() {
    ...
    try {
      ...
      const webhook_removed=await ensurePollingMode(gen);
      if(!alive(gen)) return {ok:false,stopped:true};
      await initializeCursor(gen);
      if(!alive(gen)) return {ok:false,stopped:true};
      schedule(0);
      return {ok:true,webhook_removed};
    } catch(e) {
      failures++;
      if(terminalStatus(e)) {
        running=false; generation++;
