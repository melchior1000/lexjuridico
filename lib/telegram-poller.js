...
function createTelegramPoller({
  ...
  async function tick() {
    ...
    try {
      ...
      if(is409){
        ...
        // Outro erro interrompe a sequência de 409: a contagem recomeça do zero.
        conflictSince=0;conflictReported=false;
        logger.warn?.('[Telegram] polling indisponivel; nova tentativa com backoff:',e.message);
      }
      schedule(retryDelay());
    } finally {
      inFlight=false;
  ...
  async function start() {
    ...
    running=true;conflictSince=0;conflictReported=false;
    const gen=++generation;
    try {
      await loadCursor();
