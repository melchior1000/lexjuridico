...
function createTelegramReception({records, owner, send, report, summary, compose}) {
  ...
  async function receive(msg) {
    ...
    return withProcessLock(locks,id,async () => {
      ...
      // A IA escreve a conversa dentro dos limites da decisão; classe, setor, escalonamento
      // e aprovação continuam do código. Sem IA (ou fora dos limites), vale a frase fixa.
      if (typeof compose === 'function' && text) {
        try {
          let history = [];
          try { history = (await records.read(keyFor(id)))?.value?.history || []; } catch { history = []; }
          const composed = await compose({text, decision, history: [...history.slice(0,-1)].reverse()});
          if (composed?.reply) decision = {...decision, reply: composed.reply, origemResposta: composed.origem};
        } catch (e) { console.warn('[Recepcao IA] Telegram: compositor falhou; resposta fixa: '+(e?.message||e)); }
      }
