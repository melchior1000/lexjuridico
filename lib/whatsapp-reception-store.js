...
function createReceptionStore({url=process.env.SUPABASE_URL||'', key=process.env.SUPABASE_KEY||'', transport=https, request}={}) {
  ...
  async function list({status='aguardando_advogado',limit=10,afterNumero}={}) {
    const pageSize=Math.max(1,Math.min(Number(limit)||10,100));
    const cursor=String(afterNumero||'').replace(/\D/g,'');
    const query={
      select:'numero,nome,ultima_mensagem,classe,status,urgente,contador,criado_em,atualizado_em,arquivado_em',
      status:'eq.'+status,
      order:cursor?'numero.asc':'urgente.desc,atualizado_em.desc,numero.asc',
      limit:String(pageSize)
    };
    if(cursor) query.numero='gt.'+cursor;
    try {
      const rows = rowsFromResult(await sb('GET',TABLE,null,query),'Recepcao WhatsApp');
      return rows;
    } catch {
      const filtered=memoryInbox().filter(x=>x.status===status && (!cursor || String(x.numero||x.number||'')>cursor));
      filtered.sort(cursor
        ? (a,b)=>String(a.numero||a.number||'').localeCompare(String(b.numero||b.number||''))
        : (a,b)=>(Number(b.urgente ?? b.urgent)-Number(a.urgente ?? a.urgent)) || String(b.atualizado_em||b.last_at||'').localeCompare(String(a.atualizado_em||a.last_at||'')) || String(a.numero||a.number||'').localeCompare(String(b.numero||b.number||'')));
      return filtered
        .slice(0,pageSize)
        .map(x=>({
  ...
  async function listEvents({since='',until='',limit=100,offset=0}={}) {
    ...
    try {
      ...
      if(since) query.criado_em='gte.'+since;
      const rows=rowsFromResult(await sb('GET',EVENTS_TABLE,null,query),'Historico recepcao WhatsApp');
      return until?rows.filter(x=>String(x.criado_em||'')<String(until)):rows;
    } catch(error) {
      // Recibos exigem histórico persistido completo. Não transforme falha do banco
      // em "sucesso" usando apenas a memória deste processo.
      const e=new Error('Histórico persistido da recepção indisponível: '+(error?.message||'falha de leitura'));
      e.cause=error;
      throw e;
    }
  }
  ...
  return {list,upsert,archive,appendEvent,history,listEvents,healthcheck,categoryFor};
}
