...
function createReceptionStore({url=process.env.SUPABASE_URL||'', key=process.env.SUPABASE_KEY||'', transport=https, request}={}) {
  ...
  async function list({status='aguardando_advogado',limit=10,offset=0}={}) {
    ...
    try {
      const rows = rowsFromResult(await sb('GET',TABLE,null,{
        ...
        status:'eq.'+status,
        order:'urgente.desc,atualizado_em.desc,numero.asc',
      ...
      return memoryInbox().filter(x=>x.status===status)
        .sort((a,b)=>(Number(b.urgente ?? b.urgent)-Number(a.urgente ?? a.urgent)) || String(b.atualizado_em||b.last_at||'').localeCompare(String(a.atualizado_em||a.last_at||'')) || String(a.numero||a.number||'').localeCompare(String(b.numero||b.number||'')))
