...
function createReceptionStore({url=process.env.SUPABASE_URL||'', key=process.env.SUPABASE_KEY||'', transport=https, request}={}) {
  ...
  async function findWaitingByName(nome) {
    ...
    const matches = [];
    let cursor = '';
    // A lista de exibição e o cache em memória são parciais: não provam unicidade.
    // Avança pela chave estável, mesmo quando o servidor entrega páginas menores.
    for (;;) {
      const rows = rowsFromResult(await sb('GET', TABLE, null, {
        select: 'numero,nome,classe',
        status: 'eq.aguardando_advogado',
        order: 'numero.asc',
        limit: '100',
        ...(cursor ? {numero: 'gt.' + cursor} : {})
      }), 'Busca de destinatario WhatsApp');
      if (!rows.length) return matches;
      for (const row of rows) {
        if (typeof row?.numero !== 'string' || !/^\d+$/.test(row.numero) || row.numero <= cursor) {
          throw new Error('Busca de destinatario WhatsApp: pagina invalida ou sem progresso.');
        }
        cursor = row.numero;
  ...
  return {list,findWaitingByName,upsert,archive,appendEvent,history,healthcheck,categoryFor};
}
