...
function backButton(){return '<button class="lex-back-button" onclick="lexBack()" aria-label="Voltar">‹</button>'}
...
function deadlineOrganizeCommand(){
  ...
  const rows=critical.map(x=>{const id=(x.p.numero||'sem número')+' · '+(x.p.nome||x.p.partes||'Processo');if(!x.confirmed)return'- '+id+' · PRAZO LEGADO A CONFERIR · data cadastrada '+(x.p.prazoReal||x.p.prazo||'não informada');return'- '+id+' · '+(x.d<0?'VENCIDO HÁ '+Math.abs(x.d)+' DIAS':x.d===0?'VENCE HOJE':'VENCE EM '+x.d+' DIAS')}).join('\n');
...
window.lexProcessos=function(){
  ...
  const body='<div class="lex-page-head"><div class="lex-page-title-with-back">'+backButton()+'<div><small>Carteira jurídica</small><h1>Processos</h1></div></div><button onclick="goLex(\'autuacao\')" aria-label="Cadastrar processo">＋</button></div>'
