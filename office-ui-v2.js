...
function deadlineConfirmed(p){return !!(p?.deadline_truth?.legal_truth===true&&p?.prazo_confirmado_em)}
function deadlineItems(){return procs().map(p=>({p,d:days(p),confirmed:deadlineConfirmed(p)})).filter(x=>x.d<9999).sort((a,b)=>a.d-b.d)}
