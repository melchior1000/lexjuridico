...
for(const name of ['bot.js','lex_agente_vivo.js','lex_agente_vivo_core.js','office-ui.js','office-ui-base.js','office-ui-device.js','office-ui-v2.js','login-theme.js','conector-navegador/popup.js',...['lib','scripts','test'].flatMap(dir=>fs.readdirSync(dir).filter(f=>f.endsWith('.js')).map(f=>path.join(dir,f)))]) check(name,null,name);
