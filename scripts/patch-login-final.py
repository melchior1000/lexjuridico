...
if n!=1:
    raise SystemExit(f'CUT4 replacements={n}')
start=s.index('<!-- TELA DE LOGIN -->')
end=s.index('<div id="sidebar">',start)
login='''<!-- TELA DE LOGIN -->
...
block=s[s.index('<!-- TELA DE LOGIN -->'):s.index('<div id="sidebar">')]
for token in ['background:#07070f','background:#0d0d1a','color:#e8eaf6','color:#555878']:
    if token in block:
        raise SystemExit('residuo '+token)
if 'LOGIN CLARO COMERCIAL — CUT 4' in s:
    raise SystemExit('CUT4 ainda presente')
p.write_text(s)
