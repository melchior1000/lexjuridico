...
test('HOJE é fila de exceções e não dashboard de KPI',()=>{assert.match(core,/Só mostramos decisões e exceções reais/);assert.match(core,/Prazo crítico|Prazo cadastrado/);assert.match(core,/Cliente aguardando/);assert.match(core,/Documento em quarentena/);assert.doesNotMatch(core,/lex-kpis|Setores do Escritório|Olá, Dr\./)});
...
test('prazo cadastrado não é tratado silenciosamente como verdade jurídica',()=>{assert.match(core,/Confirme a situação jurídica antes de agir/)});
