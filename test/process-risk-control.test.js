...
test('controle de risco preserva prazo e ultimo ato sem inventar dados',()=>{
  ...
  assert.equal(ctx.prazo,'17/09/2026');
...
test('triagem obriga verificar prazo, ultimo ato, parte contraria e preclusao antes de redigir',async()=>{
  ...
  const ai=async(messages,system)=>{prompts.push({messages,system});return JSON.stringify({cabivel:false,motivos:'Falta confirmar pedido adverso',faltantes:['petição da parte contrária'],riscos:['preclusão'],prazo_critico:'17/09/2026',ultimo_ato_confirmado:'Despacho de 12/09/2026',proxima_acao:'obter petição adversa'});};
  ...
  assert.match(prompts[0].messages[0].content,/"prazo":"17\/09\/2026"/);
  assert.equal(result.controle_risco.prazo,'17/09/2026');
